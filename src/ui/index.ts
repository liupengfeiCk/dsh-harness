...
import { createAgentPresetEditWire } from './wire-client.ts'
...
export {
  draftBlocker, type AgentPresetSectionState, type CatalogDraft, type CopyDraft, type DelegationDraft,
  type DelegationKey, type EditDraft, type PresetRow, type PresetView, type ToolDraft,
} from './section-store.ts'
export type { AgentPresetOption, AgentPresetSettingsState } from './settings-store.ts'
export { AGENT_PRESET_SETTINGS_NS, writeDefaultPreset } from './settings-store.ts'
export type { AgentPresetEditWire, WireCatalogTool, WireDelegationRow, WireToolRow } from './wire-client.ts'
export { AGENT_PRESET_EDIT_CHANNEL, createAgentPresetEditWire } from './wire-client.ts'
...
export function apply(ctx: ClientContext): void {
  ...
  const section = new AgentPresetSectionController(
    api,
    // The structured edit read/write ride the dedicated editing wire channel,
    // not the shared `/api` agentPreset domain.
    createAgentPresetEditWire(rpc),
    () => {
  ...
  ctx.effect(() => ctx.locale.register('settings.agentPreset', { zh, en }), 'ui-agent-preset-editing: settings row dictionaries')
