...
export interface AgentPresetEditWire {
  readEditable(payload: { agentPreset: string }, signal?: AbortSignal): Promise<RpcResult<{
    agentPreset: string
    persona?: { text: string }
  ...
  }, signal?: AbortSignal): Promise<RpcResult<{ agentPreset: string }>>
}

/** Build the editing wire face over one connection RPC caller. */
export function createAgentPresetEditWire(rpc: ClientConnectionRpc): AgentPresetEditWire {
  return {
    ...
    update: (payload, signal) => call<{ agentPreset: string }>('update', payload, signal),
  }
  ...
  function call<T>(endpoint: string, payload: unknown, signal?: AbortSignal): Promise<RpcResult<T>> {
    return rpc.call(AGENT_PRESET_EDIT_CHANNEL, endpoint, payload, signal) as Promise<RpcResult<T>>
  }
}
