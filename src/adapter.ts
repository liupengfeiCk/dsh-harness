...
/**
 * A JSON-serializable value: the value type of the optional `extra` passthrough
 * bag this replacement adapter forwards to the wire. The upstream llm core's
 * published `GenerateOptions` does not yet expose the field (the batch-2 core
 * addition lives only in the working tree), so the adapter types the bag
 * locally and reads it via a type assertion — the runtime contract is stable.
 * This mirrors the typed-escape hatch `harness/bundle/model-plan/src/merge.ts`
 * uses for `LlmCallConfig.extra` (`MergedCallConfig`): the adapter is written
 * against the contract, and the integration that lands the field consumes it
 * unchanged.
 */
type JsonValue =
  | string
...
/**
 * `GenerateOptions` plus the optional `extra` passthrough bag this adapter
 * forwards to the wire body. Intersected rather than asserted into the method
 * signature so the overridden `stream()` still satisfies the published
 * `LlmAdapter` abstract type; the bag is read at runtime when present.
 */
type GenerateOptionsWithExtra = GenerateOptions & { extra?: Record<string, JsonValue> }
...
): ((payload: unknown, _model: Model<Api>) => unknown) | undefined {
  if (extra === undefined || Object.keys(extra).length === 0) return undefined
  return (payload) => {
    if (typeof payload !== 'object' || payload === null) return payload
    const target = { ...(payload as Record<string, unknown>) }
    for (const [key, value] of Object.entries(extra)) {
      // Native fields (already on the SDK-built payload) win a collision with
      // the extra bag, exactly as the llm-deepseek serialize layer does.
...
export class PiAiAdapter extends LlmAdapter {
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    try {
      ...
      // Replacement-adapter addition: inject the `LlmCallConfig.extra`
      // passthrough bag (model-plan custom params such as `top_p`) into the
      // provider payload at pi-ai's sanctioned onPayload hook. The bag is read
      // via `GenerateOptionsWithExtra` because the published GenerateOptions
      // type has not yet grown the batch-2 `extra` field.
      const onPayload = payloadPatch((options as GenerateOptionsWithExtra).extra)
