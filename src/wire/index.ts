...
import { z } from 'zod'
import { RestartRequiredError } from '../patch.ts'
...
export async function dispatchHarnessHot(
  service: HarnessHotService | undefined,
  endpoint: string,
  payload: unknown,
  signal: AbortSignal,
  host: HarnessHotHost = {},
...
function walkError(error: unknown, out: string[], depth: number): void {
  ...
  const children: unknown[] = error instanceof AggregateError
    ? error.errors
...
async function mount(
  service: HarnessHotService,
  request: EndpointPayload<'mount'>,
  signal: AbortSignal,
): Promise<RpcResult<unknown>> {
...
async function unmount(
  service: HarnessHotService,
  request: EndpointPayload<'unmount'>,
  signal: AbortSignal,
): Promise<RpcResult<unknown>> {
...
async function upgrade(
  service: HarnessHotService,
  request: EndpointPayload<'upgrade'>,
  signal: AbortSignal,
): Promise<RpcResult<unknown>> {
 ...
 */
export function registerHarnessHotWire(ctx: Context): void {
...
/** The wire's own failure vocabulary, shaped as the carrier's RpcError. */
