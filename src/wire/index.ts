 ...
 */
export async function dispatchModelPlan(
  plans: ModelPlans | undefined,
  endpoint: string,
  payload: unknown,
  signal: AbortSignal,
 ...
 */
export function registerModelPlanWire(ctx: Context): void {
    ...
    connectionCtx.effect(
      () => connection.rpc.handle(
        ...
        (endpoint, payload, signal) =>
          dispatchModelPlan(connectionCtx.get('modelPlans'), endpoint, payload, signal),
        { authority: AUTHORITY },
      ),
      'dsh-model-plan: /model-plan rpc channel',
    )
  })
}
...
async function select(
  plans: ModelPlans | undefined,
  request: EndpointPayload<'select'>,
): Promise<RpcResult<unknown>> {
  if (plans === undefined) return fail(noPlans(request.sessionId))
  try {
    const state = await plans.select(
      request.sessionId as never,
      request.planId,
      request.overrides === undefined ? undefined : toParams(request.overrides),
    )
    return ok({
...
async function readSelection(
  plans: ModelPlans | undefined,
  request: EndpointPayload<'readSelection'>,
): Promise<RpcResult<unknown>> {
  if (plans === undefined) return fail(noPlans(request.sessionId))
  try {
    const state = await plans.readSelection(request.sessionId as never)
    return ok({
 ...
 */
function toParams(bag: Record<string | number | symbol, unknown>): PlanParams {
  return bag as PlanParams
}
...
function noPlans(id: string): WireError {
  return {
...
function planFailure(plan: string, error: unknown): WireError {
...
function selectionFailure(sessionId: string, error: unknown): WireError {
  if (error instanceof UnknownPlanError) {
    return {
      code: 'model-plan-not-found',
      message: error.message,
