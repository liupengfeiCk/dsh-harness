...
export const en: Record<TeamSettingsKey, string> = {
  ...
  sectionIntro:
    ...
    + 'Each role binds a prompt (the role\'s own) to a subagent (a user-defined subagent id '
  ...
  detailIntro:
    'Each role binds a prompt (its own, injected as the child\'s persona) to a subagent '
...
export const zh: Record<TeamSettingsKey, string> = {
  nav: '团队',
  sectionIntro:
    '团队是你自定义的角色编制表，与子代理分开存储。每个角色把提示词（角色自己的）'
    + '绑定到一个子代理（一个用户自定义子代理 id，界定该角色的能力边界）。团队形态的部署按名称把任务委派给角色。',
  loading: '正在加载团队…',
  error: '无法加载团队。',
  ...
  detailTitle: '编辑团队',
  detailIntro:
    '每个角色把提示词（自己的，注入为子代理的 persona）绑定到一个子代理（界定该角色能力范围的子代理 id）。'
