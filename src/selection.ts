...
declare module '@deepseek-ai/dsh-session/types' {
  interface SessionEventMap {
     ...
     */
    'model-plan/select': { planId?: string; overrides?: PlanParams }
  }
}

/** The empty selection: a session that binds no plan. */
export const NO_PLAN_SELECTION: PlanSelectionState = { overrides: {} }
 ...
 */
export function currentPlanSelection(events: readonly SessionEvent[]): PlanSelectionState {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    const event = events[index] as SessionEvent | undefined
    if (event?.type === 'model-plan/select') {
      const data = event.data as { planId?: string; overrides?: PlanParams }
      return {
 ...
 */
export function planLocked(events: readonly SessionEvent[]): boolean {
