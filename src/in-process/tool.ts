...
// Type-only: make `ctx.get('subagentPresets')` resolve to the user-defined
// subagent registry when composed — a delegation tool resolves a named
// subagent (a user-defined helper agent, stored fully separately from the
// agent-preset roster) opportunistically (the documented `ctx.get` pattern),
...
async function resolveSubagent(
  ...
  if (subagents === undefined) {
    throw new Error(
      `tool-subagent: subagent "${template}" requested but no subagent registry is loaded `
  ...
  if (subagent.broken !== undefined) {
    throw new Error(
      `tool-subagent: subagent "${template}" cannot be mounted: ${subagent.broken}`,
    )
  }
  if (subagent.metadata.enabled === false) {
    throw new Error(
      `tool-subagent: subagent "${template}" is disabled; enable it in the subagents settings to delegate to it`,
    )
  }
  return subagent.id
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  const refreshSubagentCatalogue = (): void => {
    const subagents = ctx.get('subagentPresets')
    if (subagents === undefined) {
      subagentCatalogue = ''
      return
  ...
  const mount = (provider: SubagentProvider): void => {
    ...
    disposeTool = ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        if (childSubagent !== undefined) refreshSubagentCatalogue()
