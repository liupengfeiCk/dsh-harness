...
function bagToExtra(bag) {
	const extra = {};
	for (const [key, value] of Object.entries(bag)) extra[key] = value;
	return extra;
}
/**
...
function createMergeHandler(deps) {
	return async (payload, next) => {
		...
		if (warnedExtra.length > 0) deps.logger.warn(`model-plans: plan "${planId ?? "default"}" carries params (${warnedExtra.join(", ")}) that land on LlmCallConfig.extra — the active provider adapter must forward them to the request body (enable the extra-forwarding adapter for the provider route)`);
		return config;
	};
}
//#endregion
