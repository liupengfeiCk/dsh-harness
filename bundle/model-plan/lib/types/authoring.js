...
import { PLAN_FILE, readPlan } from "./discovery.js";
...
export class InvalidPlanIdError extends Error {
    ...
    /** The rejected id. */
    planId) {
        super(`model-plans: plan id ${JSON.stringify(planId)} must match ${String(PLAN_ID)} — `
...
export class PlanExistsError extends Error {
    ...
    planId) {
        super(`model-plans: plan "${planId}" already exists — `
 ...
 */
export async function createPlan(roots, id, edits) {
    assertUsableId(id);
 ...
 */
export async function updatePlan(roots, plan, edits) {
    if (plan.trust !== 'user') {
 ...
 * root.
 */
export async function deletePlan(roots, plan) {
    if (plan.trust !== 'user') {
        throw new PlanNotWritableError(plan.id, 'it ships with the deployment');
    }
    const dir = join(writableRoot(roots), plan.id);
 ...
 * root.
 */
export async function setPlanDefault(roots, plan) {
    if (plan.trust !== 'user') {
        throw new PlanNotWritableError(plan.id, 'it ships with the deployment');
    }
    const dir = join(writableRoot(roots), plan.id);
