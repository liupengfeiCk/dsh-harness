...
import { PLAN_FILE } from './discovery.ts'
import type { ModelPlan, PlanParams, PlanTrust } from './types.ts'
...
function paramsOf(value: unknown): { params: PlanParams; broken?: string } | undefined {
  ...
  return { params: value as PlanParams }
}
 ...
 */
export function parsePlan(id: string, trust: PlanTrust, path: string, content: string): ModelPlan {
  let parsed: unknown
  try {
 ...
 */
export function defaultPlan(plans: readonly ModelPlan[]): ModelPlan | undefined {
