...
describe('serializeRequest', () => {
  ...
  // HARSH-owned delta: the model-plan `LlmCallConfig.extra` passthrough bag
  // must reach the wire request TOP LEVEL verbatim — "把填的字段，原样发给 LLM".
  // The official adapter dropped `extra` entirely; this replacement spreads it.
  it('spreads the extra passthrough bag into the wire request top level verbatim', () => {
    const wire = serializeRequest(request({
      messages: history,
      extra: { top_p: 0.8, max_tokens: 100, custom: { nested: true } },
    }))
    expect(wire.top_p).toBe(0.8)
    expect(wire.max_tokens).toBe(100)
    expect(wire.custom).toEqual({ nested: true })
  })

  it('lets native wire fields win when an extra key collides with one', () => {
    // The bag is spread FIRST and the native fields LAST in the object literal,
    // so a plan key colliding with a field the adapter genuinely owns never
    // hijacks it (e.g. a plan that also declares temperature/max_tokens).
    const wire = serializeRequest(request({
      messages: history,
      temperature: 0.2,
      maxTokens: 100,
      extra: { temperature: 0.9, max_tokens: 999, top_p: 0.5 },
    }))
    // Native fields win the collision.
    expect(wire.temperature).toBe(0.2)
    expect(wire.max_tokens).toBe(100)
    // The non-colliding extra key still passes through.
    expect(wire.top_p).toBe(0.5)
  })

  it('omits extra when the bag is empty or absent', () => {
    expect(serializeRequest(request({ messages: history, extra: {} })).top_p).toBeUndefined()
    expect(serializeRequest(request({ messages: history })).top_p).toBeUndefined()
  })
})
