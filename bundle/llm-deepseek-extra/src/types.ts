...
export interface WireRequest {
  ...
  stop?: string[]
  /**
   * Index signature: any model-plan extra key the adapter is asked to forward
   * lands here (spread in serialize.ts). A key colliding with a native wire
   * field is a plan bug — the native field wins because the native fields are
   * written last in the serialize object literal.
   */
  [key: string]: unknown
}
