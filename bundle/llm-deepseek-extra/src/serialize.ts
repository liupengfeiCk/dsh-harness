 ...
 *
 * HARSH-owned delta over the official copy: `serializeRequest` ALSO spreads the
 * `GenerateOptions.extra` passthrough bag (the model-plan params) into the wire
 * request TOP LEVEL verbatim. The official adapter drops `extra` entirely, so a
 * model-plan bound to the DeepSeek official route could never push
 * provider-specific keys (e.g. `top_p`, `max_tokens`) through. Here the bag is
 * spread FIRST and the native fields are written LAST, so a key colliding with
 * a native wire field is resolved in the native field's favour — the plan's
 * verbatim passthrough never hijacks a field the adapter genuinely owns.
 *
 * @module dsh-llm-deepseek-extra/serialize
 */
...
/**
 * A JSON value: the widest shape the model-plan params bag may carry. Defined
 * locally because the published `@deepseek-ai/dsh-llm` types do not yet export
 * a `JsonValue` — the upstream `LlmCallConfig.extra` contract is still landing.
 */
type JsonValue =
  | string
  | number
...
/**
 * `GenerateOptions` with the optional passthrough bag the model-plan merge
 * attaches at runtime. The published `@deepseek-ai/dsh-llm` type does not yet
 * declare `extra`; the agent-loop's `buildRequest` spreads the header config
 * (including a runtime `extra`) into `GenerateOptions`, so the adapter reaches
 * it through this cast rather than through a nonexistent type member.
 */
interface GenerateOptionsWithExtra extends GenerateOptions {
  /** The verbatim model-plan params bag, present only when the request bound one. */
...
/** The extra bag, narrowed to the JSON values `WireRequest` can carry. */
function extraBag(options: GenerateOptions): Record<string, JsonValue> {
  const withExtra = options as GenerateOptionsWithExtra
 ...
 *
 * HARSH delta: the `options.extra` bag is spread into the TOP LEVEL of the
 * returned body FIRST (verbatim — "把填的字段，原样发给 LLM"); the native
 * fields are written LAST, so a plan key colliding with a native wire field
 * never hijacks it (the native field wins).
...
export function serializeRequest(
  ...
  return {
    // The model-plan extra bag rides verbatim in the top level. Spreading it
    // FIRST means the native fields below always win a name collision.
    ...extraBag(options),
    model: options.model,
    messages,
    stream: true,
