...
import { Button, IconBrowseOutline16, IconChevronDownOutline14, IconCopyOutline16, IconEditOutline16, IconFolderOpenOutline16, IconPlusOutline16, IconTrashOutline16, Menu, Modal, Tooltip, } from '@deepseek-ai/dsh-client-ui-primitives';
...
const MAX_DEPTH_CHOICES = ['0', '1', '2', '3', '5', '10', 'provider-managed'];
...
function TextEditField({ label, value, placeholder, onEdit, textarea = false, }) {
    ...
    return (_jsxs("label", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: label }), textarea
                ? (_jsx("textarea", { className: className, value: value, spellCheck: false, placeholder: placeholder, onChange: (event) => { onEdit(event.target.value); } }))
                : (_jsx("input", { className: className, value: value, spellCheck: false, placeholder: placeholder, onChange: (event) => { onEdit(event.target.value); } }))] }));
}
/**
...
function SelectField({ label, value, choices, onSelect, disabled = false, }) {
    ...
    return (_jsxs("div", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: label }), _jsx(Menu, { open: disabled ? false : open, onClose: () => { setOpen(false); }, selectedId: value, items: choices.map(choice => ({ id: choice, label: choice })), onSelect: (choice) => { onSelect(choice); setOpen(false); }, anchor: (_jsxs("button", { type: "button", className: `${css.input} ${css.selectTrigger}`, "aria-haspopup": "menu", "aria-expanded": disabled ? false : open, disabled: disabled, onClick: () => { setOpen(!open); }, children: [_jsx("span", { children: value }), _jsx(IconChevronDownOutline14, { className: css.selectChevron })] })) })] }));
}
/** Render an on/off toggle field (a switch-styled checkbox). */
function ToggleField({ label, checked, onToggle, disabled = false, }) {
    return (_jsxs("label", { className: `${css.field} ${css.toggleField}`, children: [_jsx("span", { className: css.fieldLabel, children: label }), _jsx("input", { type: "checkbox", role: "switch", className: css.toggle, checked: checked, disabled: disabled, onChange: (event) => { onToggle(event.target.checked); } })] }));
}
...
function DelegationRow({ instance, t, actions, }) {
    ...
    return (_jsxs("div", { className: css.delegationRow, children: [_jsxs("div", { className: css.delegationHead, children: [_jsx("span", { className: css.delegationId, children: instance.id }), instance.provider === undefined ? null : (_jsxs("span", { className: css.delegationProvider, children: [t('delegation.provider'), ": ", instance.provider] }))] }), instance.backgroundMode === undefined ? null : (_jsx(SelectField, { label: t('delegation.backgroundMode'), value: instance.backgroundMode, choices: BACKGROUND_MODES, disabled: instance.backgroundModeLocked, onSelect: (value) => { actions.setEditField(scope, 'backgroundMode', value); } })), instance.enableRunInBackground === undefined ? null : (_jsx(ToggleField, { label: t('delegation.enableRunInBackground'), checked: instance.enableRunInBackground, onToggle: (checked) => { actions.setEditField(scope, 'enableRunInBackground', checked); } })), instance.maxDepth === undefined ? null : (_jsx(SelectField, { label: t('delegation.maxDepth'), value: instance.maxDepth, choices: MAX_DEPTH_CHOICES, onSelect: (value) => { actions.setEditField(scope, 'maxDepth', value); } })), _jsx(ToggleField, { label: t('delegation.enabled'), checked: instance.enabled, onToggle: (checked) => { actions.setEditField(scope, 'enabled', checked); } })] }));
}
...
function CatalogRowView({ entry, edit, t, actions, }) {
    ...
    if (entry.installed) {
        ...
        return (_jsxs("div", { className: `${css.field} ${css.toolRow}`, children: [_jsx("input", { type: "checkbox", role: "switch", "aria-label": entry.name, className: css.toggle, checked: edit.removeTools.has(entry.name) === false, disabled: !removable, onChange: (event) => { actions.setEditField(scope, 'remove', !event.target.checked); } }), _jsx("span", { className: css.toolId, children: entry.name }), _jsx("span", { className: css.toolState, children: t('toolInstalled') }), !removable ? _jsx("span", { className: css.toolExpr, children: t('toolExprDisabled') }) : null, entry.description === undefined
    ...
    return (_jsxs("div", { className: `${css.field} ${css.toolRow}`, children: [_jsx("input", { type: "checkbox", role: "switch", "aria-label": entry.name, className: css.toggle, checked: edit.installTools.has(entry.name), onChange: (event) => { actions.setEditField(scope, 'install', event.target.checked); } }), _jsx("span", { className: css.toolId, children: entry.name }), _jsx("span", { className: css.toolState, children: t('toolUninstalled') }), entry.description === undefined
                ? _jsx("span", { className: css.toolInfoSpacer })
                : (_jsx(ToolInfo, { title: entry.name, children: _jsx("span", { className: css.toolDesc, children: entry.description }) }))] }));
}
/**
...
function EditDialog({ state, t, actions }) {
    ...
    return (_jsx(Modal, { open: draft !== null, onClose: () => { actions.cancelEdit(); }, title: draft === null ? t('editTitle') : `${t('editTitle')} · ${draft.title}`, closeLabel: t('close'), description: t('editIntro'), className: css.dialog, contentClassName: css.dialogScroll, footer: (_jsxs(_Fragment, { children: [_jsx(Button, { variant: "outline", disabled: draft?.saving === true, onClick: () => { actions.cancelEdit(); }, children: t('cancel') }), _jsx(Button, { disabled: draft === null || draft.saving, onClick: () => { void actions.confirmEdit(); }, children: draft?.saving === true ? t('saving') : t('save') })] })), children: draft === null
            ? null
            : (_jsxs("div", { className: css.dialogFields, children: [draft.name === undefined ? null : (_jsx(TextEditField, { label: t('displayName'), value: draft.name, placeholder: t('displayNamePlaceholder'), onEdit: (text) => { actions.setEditField('name', 'value', text); } })), draft.description === undefined ? null : (_jsx(TextEditField, { label: t('descriptionLabel'), value: draft.description, placeholder: t('descriptionPlaceholder'), textarea: true, onEdit: (text) => { actions.setEditField('description', 'value', text); } })), draft.persona === undefined ? null : (_jsx(TextEditField, { label: t('personaText'), value: draft.persona, placeholder: t('personaTextPlaceholder'), textarea: true, onEdit: (text) => { actions.setEditField('persona', 'value', text); } })), draft.delegation.length === 0 ? null : (_jsxs("section", { className: css.editorBlock, children: [_jsx("h4", { className: css.blockTitle, children: t('delegationTitle') }), draft.delegation.map(instance => (_jsx(DelegationRow, { instance: instance, t: t, actions: actions }, instance.id)))] })), draft.catalog.length === 0 ? null : (_jsxs("section", { className: css.editorBlock, children: [_jsx("h4", { className: css.blockTitle, children: t('toolsTitle') }), draft.catalog.map(entry => (_jsx(CatalogRowView, { entry: entry, edit: draft, t: t, actions: actions }, entry.name)))] })), message === null ? null : _jsx("p", { className: css.error, role: "alert", children: message })] })) }));
}
/**
    ...
    return (_jsxs("div", { className: css.section, children: [_jsx("h2", { className: css.title, children: t('nav') }), _jsx("p", { className: css.intro, children: t('sectionIntro') }), state.error === null ? null : _jsx("p", { className: css.error, role: "alert", children: state.error }), [['system', t('builtInGroup')], ['user', t('customGroup')]].map(([trust, heading]) => {
                ...
                return (_jsxs("section", { className: css.group, children: [_jsx("h3", { className: css.groupHead, children: heading }), group.length === 0 ? null : (_jsx("ul", { className: css.cards, children: group.map(({ row, text }) => (_jsxs("li", { className: row.broken !== undefined
                                    ...
                                    : row.isDefault ? `${css.card} ${css.cardActive}` : css.card, children: [_jsxs("button", { type: "button", className: css.cardMain, "aria-pressed": row.isDefault, disabled: row.isDefault || row.broken !== undefined, "aria-label": `${row.broken !== undefined ? t('brokenBadge') : row.isDefault ? t('inUse') : t('setDefault')}: ${text.name}`, title: row.broken ?? (row.isDefault ? t('inUse') : t('setDefault')), onClick: () => { void props.makeDefault(row.id); }, children: [_jsxs("span", { className: css.cardHead, children: [_jsx("span", { className: css.cardName, children: text.name }), row.broken !== undefined
                                                ...
                                                ? null
                                                : _jsx("span", { className: css.cardBrokenReason, role: "alert", children: row.broken }), _jsx("code", { className: css.cardId, children: row.id })] }), _jsxs("div", { className: css.cardFoot, children: [row.broken === undefined
                                                ...
                                                : null, _jsx("button", { type: "button", className: css.iconButton, disabled: !state.authorable || row.broken !== undefined, "data-tip": row.broken !== undefined
                                                    ? t('brokenNoCopy')
                                                ...
                                                ? (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", className: css.iconButton, "data-tip": t('edit'), "aria-label": `${t('edit')}: ${text.name}`, onClick: () => { void props.beginEdit(row.id); }, children: _jsx(IconEditOutline16, {}) }), _jsx("button", { type: "button", className: `${css.iconButton} ${css.iconDanger}`, "data-tip": t('delete'), "aria-label": `${t('delete')}: ${text.name}`, onClick: () => { props.confirmDelete(row.id); }, children: _jsx(IconTrashOutline16, {}) })] }))
                ...
                } }), _jsx(EditDialog, { state: state, t: t, actions: {
                    cancelEdit: props.cancelEdit,
                    confirmEdit: props.confirmEdit,
                    setEditField: props.setEditField,
                } }), _jsx(Modal, { open: state.view !== null, onClose: () => { props.closeView(); }, title: state.view === null ? '' : `${t('view')} · ${viewedTitle}`, closeLabel: t('close'), description: t('composition'), className: css.dialog, contentClassName: css.dialogScroll, footer: (_jsx(Button, { variant: "outline", autoFocus: true, onClick: () => { props.closeView(); }, children: t('close') })), children: state.view === null
                    ? null
