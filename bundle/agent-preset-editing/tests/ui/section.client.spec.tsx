...
import { AgentPresetSection } from '../../src/ui/AgentPresetSection.tsx'
import type { AgentPresetSectionProps } from '../../src/ui/AgentPresetSection.tsx'
import type { AgentPresetSectionState, CopyDraft, EditDraft } from '../../src/ui/section-store.ts'
import { en } from '../../src/ui/locales.ts'
...
function renderSection(
  ...
  const actions = {
    ...
    cancelEdit: vi.fn(),
    setEditField: vi.fn(),
    confirmEdit: vi.fn(() => Promise.resolve()),
...
describe('the preset list', () => {
  ...
  it('offers View on every row, and location and edit only on a custom one', () => {
    renderSection()

    // Reading a composition is offered for both trust levels; only a custom
    // row carries the location and the structured edit.
    const standard = rowFor('standard')
...
describe('the structured editor', () => {
  const installed: EditDraft['catalog'][number] = {
    name: '@deepseek-ai/dsh-tool-read', toolNames: ['read'], installed: true,
  }
  const absent: EditDraft['catalog'][number] = {
    name: '@deepseek-ai/dsh-tool-fs', toolNames: ['edit', 'read'], description: 'File tools', installed: false,
  }

  it('renders installed tools as removable and absent ones as addable', () => {
    renderSection({
      edit: {
        id: 'mine',
        title: 'mine',
        delegation: [],
        tools: [{ id: 'tool-read', name: '@deepseek-ai/dsh-tool-read', enabled: true, toggleable: true }],
        catalog: [installed, absent],
    ...
    const dialog = screen.getByRole('dialog')
    expect(within(dialog).getByText(en.toolsTitle)).toBeTruthy()
    // The absent tool shows an add toggle (unchecked -> install on save).
    const add = within(dialog).getByRole('switch', { name: absent.name })
    expect(add).toHaveProperty('checked', false)
    // The installed tool shows a remove toggle (checked -> stays unless cleared).
    const remove = within(dialog).getByRole('switch', { name: installed.name })
    expect(remove).toHaveProperty('checked', true)
  })

  it('stages an add through the catalog toggle', () => {
    const actions = renderSection({
      edit: {
        id: 'mine',
        title: 'mine',
    ...
    const add = screen.getByRole('switch', { name: absent.name })
    fireEvent.click(add)

    expect(actions.setEditField).toHaveBeenCalledWith(`catalog:${absent.name}`, 'install', true)
  })

  it('stages a removal through the catalog toggle', () => {
    const actions = renderSection({
      edit: {
        id: 'mine',
        title: 'mine',
    ...
    const remove = screen.getByRole('switch', { name: installed.name })
    fireEvent.click(remove)

    expect(actions.setEditField).toHaveBeenCalledWith(`catalog:${installed.name}`, 'remove', true)
  })

  it('renders a delegation instance with its provider as identity', () => {
    renderSection({
      edit: {
        id: 'mine',
        title: 'mine',
        delegation: [{
          id: 'tool-subagent', enabled: true, backgroundModeLocked: false,
          provider: 'spawn', backgroundMode: 'one-shot', enableRunInBackground: true, maxDepth: '3',
    ...
    const dialog = screen.getByRole('dialog')
    expect(within(dialog).getByText(en.delegationTitle)).toBeTruthy()
    expect(within(dialog).getByText('tool-subagent')).toBeTruthy()
    expect(within(dialog).getByText(`${en['delegation.provider']}: spawn`)).toBeTruthy()
  })
})
