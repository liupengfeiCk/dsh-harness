...
import type { AgentPresetEditWire } from '../../src/ui/wire-client.ts'
import { AgentPresetSectionController, draftBlocker } from '../../src/ui/section-store.ts'
import type { CopyDraft, EditDraft, PresetRow } from '../../src/ui/section-store.ts'
...
interface FakeOptions {
  ...
  failRemove?: string
  /** Reject `readEditable` with this message. */
  failEditRead?: string
  /** Reject `update` with this message. */
  failUpdate?: string
  ...
  throwOpen?: boolean
  /** Throw from `readEditable`, as a dead transport does. */
  throwEditRead?: boolean
...
/**
 * A fake editing wire over the same in-memory store: readEditable mirrors a
 * locally authored preset's structured fields, and update folds the staged
 * metadata back into the store so the roster the controller re-reads reflects
 * the save.
 * @param presets - the same in-memory store the roster reads.
 * @param options - failure injection and call recording.
 * @returns the editing wire face.
 */
function fakeEditWire(
  presets: Map<string, FakePreset>,
  options: FakeOptions = {},
): AgentPresetEditWire {
  const record = (method: string, payload: unknown): void => { options.calls?.push({ method, payload }) }
  ...
  const fail = (message: string) =>
    Promise.resolve({ ok: false as const, error: { code: 'internal' as const, message, details: {} } })
  return {
    readEditable: (payload) => {
      record('readEditable', payload)
      if (options.throwEditRead === true) return Promise.reject(new Error('socket closed'))
      if (options.failEditRead !== undefined) return fail(options.failEditRead)
      const preset = presets.get(payload.agentPreset)
      if (preset === undefined || preset.trust === 'system') {
        return ok({
          agentPreset: payload.agentPreset,
          delegation: [], tools: [], catalog: [],
        })
      }
      return ok({
        agentPreset: payload.agentPreset,
        ...preset.persona === undefined ? {} : { persona: { text: preset.persona } },
    ...
    update: (payload) => {
      ...
      const preset = presets.get(payload.agentPreset)
      if (preset === undefined) return fail(`unknown preset ${payload.agentPreset}`)
      if (preset.trust === 'system') return fail('ships with the deployment')
      if (payload.persona !== undefined && payload.persona.text !== undefined) preset.persona = payload.persona.text
      if (payload.metadata !== undefined) {
        if (payload.metadata.name !== undefined) preset.name = payload.metadata.name
        if (payload.metadata.description !== undefined) preset.description = payload.metadata.description
      }
      return ok({ agentPreset: payload.agentPreset })
    },
  } as AgentPresetEditWire
}
...
function harness(options: FakeOptions = {}) {
  ...
  const controller = new AgentPresetSectionController(
    ...
    fakeEditWire(presets, { ...options, calls: options.calls ?? calls }),
    () => { rosterChanges += 1 },
  )
...
describe('deleting', () => {
  ...
  it('folds a dead transport into the same error surface', async () => {
    ...
    const broken = new AgentPresetSectionController(
      ...
      fakeEditWire(new Map<string, FakePreset>()),
    )
...
describe('a controller with no roster listener', () => {
  it('completes a delete without anyone to notify', async () => {
    ...
    const alone = new AgentPresetSectionController(
      fakeApi(presets, { id: 'standard' }),
      fakeEditWire(presets),
    )
    await alone.load()
...
describe('the structured editor', () => {
  it('seeds a locally authored preset\'s editable fields', async () => {
    const { controller, presets } = harness()
    presets.set('mine', {
      trust: 'user',
      content: '',
      name: 'Mine',
      persona: 'You are a fixture.',
      delegation: [{ id: 'tool-subagent', provider: 'spawn', backgroundMode: 'one-shot' }],
      tools: [{ id: 'tool-bash', name: '@deepseek-ai/dsh-tool-bash', disabled: false }],
      catalog: [{ name: '@deepseek-ai/dsh-tool-fs', toolNames: ['edit'], installed: false }],
    })
    await controller.load()
    ...
    expect(edit?.persona).toBe('You are a fixture.')
    expect(edit?.delegation.map((d: EditDraft['delegation'][number]) => d.id)).toEqual(['tool-subagent'])
    expect(edit?.tools[0]).toMatchObject({ id: 'tool-bash', enabled: true, toggleable: true })
    expect(edit?.catalog[0]).toMatchObject({ name: '@deepseek-ai/dsh-tool-fs', installed: false })
    expect(edit?.name).toBe('Mine')
  })
  ...
  it('answers no fields for a shipped preset', async () => {
    ...
    const edit = controller.store.getSnapshot().edit
    expect(edit).not.toBeNull()
    expect(edit?.delegation).toEqual([])
    expect(edit?.persona).toBeUndefined()
  })
  ...
  it('stages fields and saves only what the user touched', async () => {
    const { controller, calls } = harness()
    await controller.load()
    await controller.beginEdit('mine')
    const edit = controller.store.getSnapshot().edit
    if (edit === null) throw new Error('expected an open editor')

    controller.setEditField('persona', 'value', 'new persona')
    controller.confirmEdit()

    // The save resolves synchronously in this fake; wait for the snapshot.
    await controller.store.getSnapshot()
    await Promise.resolve()

    const update = calls.find(call => call.method === 'update')
    expect(update).toBeDefined()
    expect((update?.payload as { persona?: { text?: string } }).persona).toEqual({ text: 'new persona' })
  })

  it('saves staged delegation fields', async () => {
    ...
    await controller.beginEdit('mine')
    const edit = controller.store.getSnapshot().edit
    const first = edit?.delegation[0]
    if (edit === null || first === undefined) throw new Error('expected delegation')

    controller.setEditField(`delegation:${first.id}`, 'backgroundMode', 'continuable')
    controller.confirmEdit()
    await Promise.resolve()

    const update = calls.find(call => call.method === 'update')
    expect(update).toBeDefined()
    expect((update?.payload as { delegation?: unknown }).delegation)
      .toEqual({ 'tool-subagent': { backgroundMode: 'continuable' } })
  })

  it('maps catalog installs and removals to installTools and removeTools', async () => {
    const { controller, calls } = harness()
    await controller.load()
    await controller.beginEdit('mine')
    const edit = controller.store.getSnapshot().edit
    ...
    controller.setEditField(`catalog:@deepseek-ai/dsh-tool-fs`, 'install', true)
    controller.confirmEdit()
    await Promise.resolve()

    const update = calls.find(call => call.method === 'update')
    expect(update).toBeDefined()
    expect((update?.payload as { installTools?: string[] }).installTools)
      .toEqual(['@deepseek-ai/dsh-tool-fs'])
  })

  it('keeps the dialog open with a save refusal on it', async () => {
    const { controller } = harness({ failUpdate: 'cannot edit' })
    await controller.load()
    await controller.beginEdit('mine')

    controller.setEditField('persona', 'value', 'x')
    controller.confirmEdit()
    await Promise.resolve()

    const edit = controller.store.getSnapshot().edit
    expect(edit?.saving).toBe(false)
    expect(edit?.error).toBe('cannot edit')
  })

  it('folds a dead transport into the dialog error on read', async () => {
    const { controller } = harness({ throwEditRead: true })
    await controller.load()
  ...
  it('leaves the editor alone until a locally authored preset opens it', async () => {
    const { controller } = harness()
    await controller.load()
