...
import { readTeam } from "./metadata.js";
import { TEAM_ID } from "./types.js";
...
export class InvalidTeamIdError extends Error {
    ...
    /** The rejected id. */
    teamId) {
        super(`team-presets: team id ${JSON.stringify(teamId)} must match ${String(TEAM_ID)} — `
...
export class TeamExistsError extends Error {
    ...
    teamId) {
        super(`team-presets: team "${teamId}" already exists — `
 ...
 */
export function renderTeam(metadata, roles) {
    const cleanMetadata = {};
    if (metadata.name !== undefined)
        cleanMetadata.name = metadata.name;
    if (metadata.description !== undefined)
        cleanMetadata.description = metadata.description;
    if (metadata.enabled !== undefined)
        cleanMetadata.enabled = metadata.enabled;
    const cleanRoles = roles.map(role => {
        const row = { id: role.id, subagent: role.subagent };
        if (role.description !== undefined)
            row.description = role.description;
        if (role.prompt !== undefined)
            row.prompt = role.prompt;
        row.memory = role.memory;
        return row;
    });
    const document = {};
    if (Object.keys(cleanMetadata).length > 0)
        document.metadata = cleanMetadata;
    if (cleanRoles.length > 0)
        document.roles = cleanRoles;
 ...
 */
export async function createTeam(roots, id, metadata, roles) {
    if (!TEAM_ID.test(id))
        throw new InvalidTeamIdError(id);
    ...
    if (await occupied(file))
        throw new TeamExistsError(id);
    try {
 ...
 */
export async function updateTeam(roots, team, metadata, roles) {
    if (team.trust !== 'user') {
 ...
 * root.
 */
export async function deleteTeam(roots, team) {
    if (team.trust !== 'user') {
        throw new TeamNotWritableError(team.id, 'it ships with the deployment');
    }
    const dir = join(writableRoot(roots), team.id);
 ...
 * root.
 */
export async function setTeamEnabled(roots, team, enabled) {
    if (team.trust !== 'user') {
        throw new TeamNotWritableError(team.id, 'it ships with the deployment');
    }
    const dir = join(writableRoot(roots), team.id);
    ...
    await writeFileAtomic(team.path, renderTeam({ ...current.metadata, enabled }, current.roles), { mode: 0o600, dirMode: 0o700 });
}
