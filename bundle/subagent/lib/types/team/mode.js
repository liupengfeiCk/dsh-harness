...
export const STANDARD_TOOLS = ['delegate', 'delegate_fork'];
 ...
 */
export function currentTeamMode(events) {
 ...
 */
export function modeLocked(events) {
 ...
 */
export function applyModeRestrictions(ctx, agent, state) {
    const disposers = [];
    const denyIfPresent = (name) => {
        ...
        if (ctx.get('tools')?.get(name, agent) === undefined)
            return;
