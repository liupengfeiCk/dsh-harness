...
function isWsl(internals: WireOpenerInternals): boolean {
...
async function openWindowsDirectory(path: string, signal: AbortSignal, run: NativeCommandRunner): Promise<void> {
...
async function openWslDirectory(path: string, signal: AbortSignal, run: NativeCommandRunner): Promise<void> {
 ...
 */
export function canOpenDirectory(internals: WireOpenerInternals = {}): boolean {
  const platform = internals.platform ?? process.platform
