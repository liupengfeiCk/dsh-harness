/**
 * Harness-owned continuable-subagent manager: stable child ids, descriptor
 ...
 * `ctx.subagents`. The service for this harness composition
 * (`HarnessSubagentRuntime` in `./service.ts`) installs THIS manager, never the
 * official `@deepseek-ai/dsh-subagent` continuation manager, so the inheritance
 ...
 * `@deepseek-ai/dsh-subagent/src/continuation.ts` so the persisted payload and
 * the residency lifecycle stay byte-identical with the official seam while the
 * harness is shipping the inheritance switch. The DELIBERATE deviations from
 * the official module are:
 *   - child composition runs through OUR `setupChildComposition` (./engine.ts)
 *     instead of the official `applyChildComposition`, so the `inheritParent`
 *     switch and subagent model override stay in this package;
 *   - descriptor snapshot/fold run through OUR `snapshotHarnessSubagentDescriptor`
 *     /`foldHarnessSubagentDescriptor` (./descriptor.ts) so the persisted
 ...
 *
 * @module dsh-harness-subagent-bundle/in-process/continuation
 */
...
import {
  foldHarnessSubagentDescriptor,
  snapshotHarnessSubagentDescriptor,
} from './descriptor.ts'
import type { HarnessSubagentDescriptorData } from './descriptor.ts'
...
import { assertSubagentMaxDepth, seedDescriptorTurn, SubagentError } from '@deepseek-ai/dsh-subagent'
import type {
...
export interface ContinuableStartSpec {
   ...
   */
  readonly request: Omit<HarnessSubagentStartRequest, 'label' | 'signal' | 'outputSchema'>
 ...
 * depending back on the whole {@link HarnessSubagentRuntime}. Exported so
 * `./service.ts` can build the host the manager needs; no other consumer
 * outside this package supplies one.
 */
...
export class SubagentContinuationManager {
  ...
  private async coldResume(
    ...
    // continuable child.
    const descriptor = foldHarnessSubagentDescriptor(loaded.events.slice(loaded.meta.seedLength ?? 0))
  ...
  private async materializeTracked(
    ...
    const setup = async (childCtx: Context): Promise<AgentSetupCommit> => {
      ...
      // Harness composition: the `inheritParent` switch and subagent model
      // override live in OUR engine, not the official `applyChildComposition`
      // (which loses them when the official package is restored upstream).
      await setupChildComposition(childCtx, parent, inputs.composition)
