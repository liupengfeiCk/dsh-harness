...
import { foldHarnessSubagentDescriptor } from '../in-process/descriptor.ts'
...
/** The resolved caller on the delegation execution gate (rule 3). */
interface CallerIdentity {
  /** The team id the caller belongs to. */
  readonly teamId: string
  /** The caller's level (the boss is the top, unrestricted level). */
  readonly level: number
  /** `boss` = the main agent (unrestricted); `role` = a team-role child. */
  readonly kind: 'boss' | 'role'
}
...
async function resolveCallerIdentity(
  ...
  if (parent.session !== undefined) {
    ...
    if (descriptor !== undefined && descriptor.team !== undefined && descriptor.role !== undefined) {
      ...
      if (role === undefined || role.broken !== undefined) return undefined
...
export function apply(ctx: Context, config: Config): void {
  ...
  // One structured roster row: enough to render the reader-aware authority
  // table (rule 4) — the boss sees every role + level, a level>=2 role sees its
  // subordinates, a level-1 role sees nothing. Keyed by team id, refreshed by
  // {@link refreshRoleCatalogue}. Lists only non-broken roles; the bare
  // subagent roster is never shown.
  interface RosterRow {
    readonly id: string
  ...
  const refreshRoleCatalogue = (): void => {
    ...
    teams.list().then((rows) => {
      ...
      // listed here.
      const nextRosters = new Map<string, RosterRow[]>()
      const nextTeamDisplayNames = new Map<string, string>()
      ...
      if (nextRosters.size > 0 || rosters.size === 0) {
        ...
        for (const [teamId, name] of nextTeamDisplayNames) teamDisplayNames.set(teamId, name)
        for (const [teamId, rowsFor] of nextRosters) rosters.set(teamId, rowsFor)
      }
    }).catch(() => {
  ...
  const mount = (provider: SubagentProvider): void => {
    ...
    disposeTool = ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        if (teams === undefined) {
          throw new Error(
            'team-tool: no team registry is loaded '
        ...
        if (caller.kind !== 'boss' && role.level >= caller.level) {
          throw new Error(
            `委派被拒：目标角色 "${role.id}" 的级别 ${role.level} 不低于调用者级别 ${caller.level}，`
            + '须严格递减（同级不可互派）',
          )
        }
  ...
  ctx.systemPrompt.section({
    ...
    text: context => {
      ...
      if (context.agent === undefined || context.agent.session === undefined) return ''
      ...
      if (descriptor !== undefined && descriptor.team !== undefined && descriptor.role !== undefined) {
        ...
        if (selfLevel === undefined || selfLevel < 2) return ''
        ...
        const subLines = subordinates.map(row =>
          `- ${row.id} (level ${row.level}): ${row.description ?? row.id}`)
