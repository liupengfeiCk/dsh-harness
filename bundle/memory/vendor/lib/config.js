...
export function parseConfig(raw) {
    ...
    const offloadMode = (() => {
        ...
        if (raw === "local" || raw === "backend" || raw === "client" || raw === "collect")
            return raw;
        // Auto-derive: if backendUrl is set → "backend"; if server.url is set → "client"; else "local"
        if (optStr(offloadGroup, "backendUrl"))
            return "backend";
        if (serverUrl)
            return "client";
        return "local";
    ...
    return {
        ...
        llm: (() => {
            ...
            const provider = rawProvider === "proxy" ? "proxy" : "openai";
...
function normalizePromptMode(value, fallback = "chat") {
