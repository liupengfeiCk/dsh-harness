...
import { RecallErrors, toRecallFailure } from "./recall-errors.js";
import { buildFtsQuery } from "../store/sqlite.js";
import { sanitizeText } from "../../utils/sanitize.js";
import path from "node:path";
import { createScopedStorageAdapter } from "../storage/adapter.js";
import { StoragePaths } from "../storage/types.js";
import { DEFAULT_PROFILE_SCOPE, buildProfileIsolationScope, } from "../profile/profile-sync.js";
...
export async function performAutoRecall(params) {
    ...
    return Promise.race([
        ...
        new Promise((resolve) => {
            timer = setTimeout(() => {
                logger?.warn?.(`${TAG} ⚠️ Recall timed out after ${timeoutMs}ms — surfacing as RecallResult.error`);
                resolve({
                    prependContext: "",
                    appendSystemContext: "",
                    recalledL1Memories: [],
...
async function performAutoRecallInner(params) {
    try {
        ...
        params.logger?.warn?.(`${TAG} recall failed: code=${re.code} category=${re.category} msg="${re.message}"`);
...
async function searchMemoriesWithDetails(userText, pluginDataDir, cfg, logger, strategy, vectorStore, embeddingService) {
    ...
    const memories = result.lines.map((line, i) => {
        const match = line.match(/^-\s+\[([^\]]+)\]\s+(.+?)(?:\s*\(活动时间:.*\))?$/);
        if (match) {
            const tag = match[1];
            const content = match[2].trim();
