 ...
 */
async function runLlmJudgment(matches, memories, config, logger, model, promptMode, llmRunner, traceContext) {
    ...
    try {
        ...
        if (llmRunner) {
            ...
            // NOTE: 原 fallback 为 OpenClaw 专用 CleanContextRunner（已随设计文档
            // §8.2 裁切）。DSH 适配层始终注入 host-neutral LLMRunner，缺 runner 时报错。
            throw new Error(`${TAG} L1 conflict detection requires an LLMRunner — DSH host must inject one ` +
 ...
 */
async function findCandidatesByVector(memories, vectorStore, embeddingService, topK, logger, embeddingTimeoutMs, filter) {
 ...
 */
async function findCandidatesByFts(memories, vectorStore, _logger, filter) {
    const newRecordIds = new Set(memories.map((m) => m.record_id));
    const matches = [];
