...
export function createScopedStorageAdapter(base, prefix) {
    if (!prefix)
        return base;
...
export class StorageAdapter {
     ...
     */
    async copyTree(srcPrefix, dstPrefix, opts = {}) {
        ...
        for (const entry of srcFiles) {
            ...
            if (rel.startsWith(srcNorm))
                rel = rel.slice(srcNorm.length);
            else if (rel === srcPrefix)
                rel = "";
