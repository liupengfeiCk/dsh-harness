...
export class VectorStore {
    ...
    `);
        ...
        try {
            this.db.exec("ALTER TABLE l1_records ADD COLUMN team_id TEXT DEFAULT 'default'");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE l1_records ADD COLUMN user_id TEXT NOT NULL DEFAULT 'default'");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE l1_records ADD COLUMN agent_id TEXT NOT NULL DEFAULT 'default'");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE l1_records ADD COLUMN task_id TEXT DEFAULT ''");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE l1_records ADD COLUMN version INTEGER NOT NULL DEFAULT 0");
        }
        catch { /* exists */ }
    ...
    `);
        ...
        try {
            this.db.exec("ALTER TABLE l0_conversations ADD COLUMN team_id TEXT DEFAULT 'default'");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE l0_conversations ADD COLUMN user_id TEXT NOT NULL DEFAULT 'default'");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE l0_conversations ADD COLUMN agent_id TEXT NOT NULL DEFAULT 'default'");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE l0_conversations ADD COLUMN task_id TEXT DEFAULT ''");
        }
        catch { /* exists */ }
        ...
        // NOTE: 原 Skill 表（skill_meta/skill_fts/skill_vec）的 schema 初始化
        // 已随 Skill 提取模块裁切移除（设计文档 §8.8 第一期不含）。L0/L1 机制不受影响。
    ...
    `);
        ...
        try {
            this.db.exec("ALTER TABLE entity_knowledge ADD COLUMN agent_id TEXT NOT NULL DEFAULT ''");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE entity_teams ADD COLUMN user_ids_json TEXT NOT NULL DEFAULT '[]'");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE entity_teams ADD COLUMN agent_ids_json TEXT NOT NULL DEFAULT '[]'");
        }
        catch { /* exists */ }
        try {
            this.db.exec("ALTER TABLE entity_tasks ADD COLUMN user_ids_json TEXT NOT NULL DEFAULT '[]'");
        }
        catch { /* exists */ }
            ...
            try {
                ...
                if (this.ftsAvailable) {
                    try {
                        ...
                        recordId, record.type, record.priority, record.scene_name, record.sessionKey, record.sessionId || DEFAULT_ISOLATION_ID, record.teamId || DEFAULT_ISOLATION_ID, record.taskId || "", record.userId || DEFAULT_ISOLATION_ID, record.agentId || DEFAULT_ISOLATION_ID, record.version ?? 0, tsStr, tsStart, tsEnd, JSON.stringify(record.metadata));
                    }
                    catch (ftsErr) {
     ...
     */
    searchL1Vector(queryEmbedding, topK = 5, _queryText, filter) {
     ...
     */
    deleteL1(recordId, filter) {
        if (this.degraded)
            return false;
        try {
            if (filter) {
                const meta = this.stmtGetMeta.get(recordId);
                if (!meta || !rowMatchesIsolation(meta, filter))
                    return false;
            }
            this.db.exec("BEGIN");
            try {
     ...
     * **Fault-tolerant**: logs a warning on failure, never throws.
     */
    deleteL1Batch(recordIds, filter) {
        if (this.degraded)
            return false;
        ...
        try {
            ...
            try {
                for (const id of recordIds) {
                    if (filter) {
                        const meta = this.stmtGetMeta.get(id);
                        if (!meta || !rowMatchesIsolation(meta, filter))
                            continue;
                    }
    ...
    queryL1Records(filter) {
        ...
        try {
            ...
            if (filter?.teamId !== undefined)
                rows = rows.filter((r) => r.team_id === filter.teamId);
            if (filter?.userId !== undefined)
                rows = rows.filter((r) => r.user_id === filter.userId);
            if (filter?.agentId !== undefined)
                rows = rows.filter((r) => r.agent_id === filter.agentId);
            if (taskId !== undefined)
                rows = rows.filter((r) => r.task_id === taskId);
            this.logger?.info(`${TAG} [L1-query] filter={sessionKey=${sessionKey ?? "(all)"}, sessionId=${sessionId ?? "(all)"}, teamId=${filter?.teamId ?? "(all)"}, userId=${filter?.userId ?? "(all)"}, agentId=${filter?.agentId ?? "(all)"}, taskId=${taskId ?? "(all)"}, updatedAfter=${updatedAfter ?? "(none)"}}, ` +
    ...
    upsertL0(record, embedding) {
        ...
        try {
            ...
            try {
                ...
                this.stmtL0UpsertMeta.run(record.id, record.sessionKey, record.sessionId || DEFAULT_ISOLATION_ID, record.teamId || DEFAULT_ISOLATION_ID, record.taskId || "", record.role, record.messageText, record.recordedAt, record.timestamp, record.userId || DEFAULT_ISOLATION_ID, record.agentId || DEFAULT_ISOLATION_ID);
                if (!skipVec) {
                    // vec0 does not support ON CONFLICT → delete then insert
                ...
                if (this.ftsAvailable) {
                    try {
                        ...
                        record.id, record.sessionKey, record.sessionId || DEFAULT_ISOLATION_ID, record.teamId || DEFAULT_ISOLATION_ID, record.taskId || "", record.userId || DEFAULT_ISOLATION_ID, record.agentId || DEFAULT_ISOLATION_ID, record.role, record.recordedAt, record.timestamp);
                    }
                    catch (ftsErr) {
                        // FTS write failure is non-fatal — log and continue
     ...
     */
    searchL0Vector(queryEmbedding, topK = 5, _queryText, filter) {
        if (this.degraded || !this.vecTablesReady) {
            if (this.degraded)
    ...
    queryL0Paginated(filter) {
        ...
        try {
            ...
            const rows = this.db.prepare(dataSql).all(...params, filter.limit, filter.offset);
            return { rows, total };
        }
        catch (err) {
     ...
     */
    deleteL0BySession(sessionId, filter) {
        ...
        try {
            ...
            const rows = this.db.prepare("SELECT record_id, session_key, session_id, team_id, task_id, user_id, agent_id FROM l0_conversations WHERE session_key = ? OR session_id = ?").all(sessionIdTrimmed, sessionIdTrimmed);
            if (rows.length === 0)
                return 0;
            this.db.exec("BEGIN");
            try {
                let deletedCount = 0;
                for (const row of rows) {
                    if (filter && !rowMatchesIsolation(row, filter))
                        continue;
                    ...
                    if (this.vecTablesReady) {
                        try {
                            this.db.prepare("DELETE FROM l0_vec WHERE record_id = ?").run(row.record_id);
                        }
                        catch { /* vec may not exist */ }
                    }
                    if (this.ftsAvailable) {
                        try {
                            this.db.prepare("DELETE FROM l0_fts WHERE record_id = ?").run(row.record_id);
                        }
    ...
    clearMemoryContent(filter) {
        ...
        const empty = { l0Deleted: 0, l1Deleted: 0, profilesDeleted: 0 };
        if (this.degraded)
            return empty;
        ...
        try {
            const l0Ids = this.db.prepare(`SELECT record_id FROM l0_conversations WHERE ${where}`).all(...params).map((r) => r.record_id);
            const l1Ids = this.db.prepare(`SELECT record_id FROM l1_records WHERE ${where}`).all(...params).map((r) => r.record_id);
            if (l0Ids.length === 0 && l1Ids.length === 0)
                return empty;
            this.db.exec("BEGIN");
            try {
                let l0Deleted = 0;
                for (const id of l0Ids) {
                    ...
                    if (this.vecTablesReady) {
                        try {
                            this.db.prepare("DELETE FROM l0_vec WHERE record_id = ?").run(id);
                        }
                        catch { /* vec may not exist */ }
                    }
                    if (this.ftsAvailable) {
                        try {
                            this.db.prepare("DELETE FROM l0_fts WHERE record_id = ?").run(id);
                        }
                        catch { /* fts may not exist */ }
                    }
                }
                ...
                for (const id of l1Ids) {
                    ...
                    if (this.vecTablesReady) {
                        try {
                            this.db.prepare("DELETE FROM l1_vec WHERE record_id = ?").run(id);
                        }
                        catch { /* vec may not exist */ }
                    }
                    if (this.ftsAvailable) {
    ...
    jsonArray(value) {
        return JSON.stringify(Array.isArray(value) ? value.filter((v) => typeof v === "string" && v.trim().length > 0) : []);
    }
    parseArray(value) {
    ...
    teamFromRow(row, includeRefs = false) {
        ...
        if (includeRefs) {
            const userIds = new Set(this.parseArray(row.user_ids_json));
            if (ownerUserId)
                userIds.add(ownerUserId);
            team.user_ids = Array.from(userIds).sort();
            const agentIds = new Set(this.parseArray(row.agent_ids_json));
            const agents = this.db.prepare("SELECT agent_id FROM entity_agents WHERE team_id = ? AND status = 'active' ORDER BY agent_id").all(teamId);
            for (const agent of agents)
                agentIds.add(String(agent.agent_id));
            team.agent_ids = Array.from(agentIds).sort();
            team.task_ids = this.db.prepare("SELECT task_id FROM entity_tasks WHERE team_id = ? ORDER BY task_id").all(teamId).map((r) => String(r.task_id));
        }
        return team;
    }
    userFromRow(row, includeDerived = false) {
        ...
        if (includeDerived) {
            const teams = this.db.prepare("SELECT team_id, owner_user_id, user_ids_json FROM entity_teams WHERE status = 'active'").all();
            user.team_ids = teams
                ...
                .sort();
            const tasks = this.db.prepare("SELECT task_id, creator_user_id, user_ids_json, agent_ids_json FROM entity_tasks ORDER BY task_id").all();
            ...
            for (const task of tasks) {
                ...
                for (const agentId of this.parseArray(task.agent_ids_json))
                    taskAgentIds.add(agentId);
            }
            user.task_ids = Array.from(taskIds).sort();
            user.task_agent_ids = Array.from(taskAgentIds).sort();
            user.owned_agent_ids = this.db.prepare("SELECT agent_id FROM entity_agents WHERE owner_user_id = ? AND status = 'active' ORDER BY agent_id").all(userId).map((r) => String(r.agent_id));
        }
        return user;
    }
    agentFromRow(row, includeDerived = true) {
        ...
        if (includeDerived) {
            const tasks = this.db.prepare("SELECT task_id, agent_ids_json FROM entity_tasks ORDER BY task_id").all();
    ...
    getTeam(teamId) {
        const row = this.db.prepare("SELECT * FROM entity_teams WHERE team_id = ?").get(teamId);
    ...
    deleteTeams(teamIds) {
        const result = { deleted_ids: [], failed: [] };
    ...
    getUser(userId) {
        const row = this.db.prepare("SELECT * FROM entity_users WHERE user_id = ?").get(userId);
    ...
    createAgent(input) {
        ...
        if (team && !(team.agent_ids ?? []).includes(id))
            this.updateTeam(input.team_id, { agent_ids: [...(team.agent_ids ?? []), id] });
    ...
    getAgent(agentId) {
        const row = this.db.prepare("SELECT * FROM entity_agents WHERE agent_id = ?").get(agentId);
    ...
    getTask(taskId) {
        const row = this.db.prepare("SELECT * FROM entity_tasks WHERE task_id = ?").get(taskId);
    ...
    createKnowledge(input) {
        ...
        const existing = this.db.prepare("SELECT created_at FROM entity_knowledge WHERE knowledge_id = ?").get(input.knowledge_id);
        if (existing) {
            this.db.prepare("UPDATE entity_knowledge SET type=?, service_url=?, name=?, summary=?, team_id=?, agent_id=?, user_id=?, repo_url=?, branch=?, updated_at=? WHERE knowledge_id=?").run(input.type, input.service_url, input.name, input.summary ?? null, input.team_id, input.agent_id ?? "", input.user_id ?? null, input.repo_url ?? null, input.branch ?? null, now, input.knowledge_id);
            return this.getKnowledge(input.knowledge_id);
        }
        this.db.prepare("INSERT INTO entity_knowledge (knowledge_id, type, service_url, name, summary, team_id, agent_id, user_id, repo_url, branch, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(input.knowledge_id, input.type, input.service_url, input.name, input.summary ?? null, input.team_id, input.agent_id ?? "", input.user_id ?? null, input.repo_url ?? null, input.branch ?? null, now, now);
        return this.getKnowledge(input.knowledge_id);
    }
    getKnowledge(knowledgeId) {
        const row = this.db.prepare("SELECT * FROM entity_knowledge WHERE knowledge_id = ?").get(knowledgeId);
    ...
    updateKnowledge(knowledgeId, patch) {
        ...
        if (patch.service_url !== undefined) {
            sets.push("service_url = ?");
            args.push(patch.service_url);
        }
        if (patch.repo_url !== undefined) {
            sets.push("repo_url = ?");
            args.push(patch.repo_url);
        }
    ...
    deleteKnowledge(knowledgeIds, teamId) {
        const result = { deleted_ids: [], failed: [] };
    ...
    listKnowledge(input) {
        ...
        const countArgs = [input.team_id];
        if (input.type) {
            countSql += " AND type = ?";
            countArgs.push(input.type);
        }
        if (idClause) {
            countSql += idClause;
            countArgs.push(...ids);
        }
        const totalRow = this.db.prepare(countSql).get(...countArgs);
     ...
     * **Fault-tolerant**: returns an empty array on any error.
     */
    searchL1Fts(ftsQuery, limit = 20, filter) {
        if (this.degraded || !this.ftsAvailable)
            return [];
        try {
            const retrieveLimit = filter ? Math.max(limit * 5, limit) : limit;
            const rows = this.stmtL1FtsSearch.all(ftsQuery, retrieveLimit);
            return rows
     ...
     * **Fault-tolerant**: returns an empty array on any error.
     */
    searchL0Fts(ftsQuery, limit = VectorStore.FTS_DEFAULT_LIMIT, filter) {
        if (this.degraded || !this.ftsAvailable)
            return [];
        try {
            const retrieveLimit = filter ? Math.max(limit * 5, limit) : limit;
            const rows = this.stmtL0FtsSearch.all(ftsQuery, retrieveLimit);
            return rows
                .filter((r) => rowMatchesIsolation(r, filter))
                .slice(0, limit)
                .map((r) => ({
        ...
        `)
            ...
            for (const r of l1Rows) {
                try {
                    ...
                    r.record_id, r.type, r.priority, r.scene_name, r.session_key, r.session_id || DEFAULT_ISOLATION_ID, r.team_id || "", r.task_id || "", r.user_id || DEFAULT_ISOLATION_ID, r.agent_id || DEFAULT_ISOLATION_ID, r.version ?? 0, r.timestamp_str, r.timestamp_start, r.timestamp_end, r.metadata_json);
                    l1Count++;
                }
                catch (err) {
        ...
        `)
            ...
            for (const r of l0Rows) {
                try {
                    ...
                    r.record_id, r.session_key, r.session_id, r.team_id ?? "", r.task_id ?? "", r.user_id ?? "", r.agent_id ?? "", r.role, r.recorded_at, r.timestamp);
                    l0Count++;
                }
                catch (err) {
    ...
    appendAudit(entry) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(entry.audit_id, entry.record_id, entry.layer, entry.action, entry.team_id ?? null, entry.agent_id ?? null, entry.user_id ?? null, entry.task_id ?? null, entry.version, entry.updated_at_ms, entry.request_id ?? null);
    }
    queryAudit(filter) {
        const conds = [];
        const args = [];
        if (filter.record_id !== undefined) {
            conds.push("record_id = ?");
            args.push(filter.record_id);
        }
        ...
        if (filter.team_id !== undefined) {
            conds.push("team_id = ?");
            args.push(filter.team_id);
        }
        if (filter.agent_id !== undefined) {
            conds.push("agent_id = ?");
            args.push(filter.agent_id);
        }
        ...
        if (filter.since_ms !== undefined) {
            conds.push("updated_at_ms >= ?");
            args.push(filter.since_ms);
        }
        if (filter.until_ms !== undefined) {
            conds.push("updated_at_ms <= ?");
            args.push(filter.until_ms);
        }
    ...
    `;
        ...
        const rows = stmt.all(...args, limit, offset);
        return rows.map((r) => ({
            audit_id: r.audit_id,
            record_id: r.record_id,
            layer: r.layer,
    ...
    // ─────────────────────────────────────────────────────────
    upsertMemoryGenerationRefs(records) {
        const stmt = this.db.prepare(`
    ...
    `);
        ...
        try {
            for (const record of records) {
                stmt.run(record.generation_ref_id, record.layer, record.memory_id, record.generation_id, record.generation_log_id, record.generation_log_key, record.memory_prompt_id, record.memory_prompt_version, record.memory_prompt_source, record.created_at_ms);
            }
            this.db.exec("COMMIT");
        }
        catch (err) {
    ...
    getMemoryGenerationRef(layer, memoryId) {
        const id = buildMemoryGenerationRefId(layer, memoryId);
        return this.db.prepare("SELECT * FROM memory_generation_refs WHERE generation_ref_id = ? AND layer = ? AND memory_id = ?").get(id, layer, memoryId) ?? null;
    }
    // ─────────────────────────────────────────────────────────
    // Custom Memory Prompt
    // ─────────────────────────────────────────────────────────
    countMemoryPrompts() {
        const row = this.db.prepare("SELECT COUNT(*) AS total FROM memory_prompts").get();
        return Number(row?.total ?? 0);
    }
    createMemoryPrompt(record) {
        this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(record.memory_prompt_id, record.name, record.layer, record.prompt, record.version, record.status, record.created_by ?? null, record.updated_by ?? null, record.created_at_ms, record.updated_at_ms);
        return record;
    }
    getMemoryPrompts(ids) {
        if (ids.length === 0)
            return [];
        const placeholders = ids.map(() => "?").join(",");
        return this.db.prepare(`SELECT * FROM memory_prompts WHERE memory_prompt_id IN (${placeholders})`).all(...ids);
    }
    listMemoryPrompts(filter) {
    ...
    updateMemoryPrompt(id, patch) {
        ...
        if (sameName && samePrompt)
            return current;
        const sets = ["version = version + 1", "updated_at_ms = ?", "updated_by = ?"];
        const args = [patch.updated_at_ms, patch.updated_by ?? null];
        if (patch.name !== undefined) {
            sets.push("name = ?");
            args.push(patch.name);
        }
        ...
        args.push(id);
        const result = this.db.prepare(`UPDATE memory_prompts SET ${sets.join(", ")} WHERE memory_prompt_id = ? AND status = 'active'`).run(...args);
        if (Number(result.changes ?? 0) === 0)
            return null;
        return this.getMemoryPrompts([id])[0] ?? null;
    }
    getMemoryPromptSettings(ids) {
        if (ids.length === 0)
            return [];
        const placeholders = ids.map(() => "?").join(",");
        return this.db.prepare(`SELECT * FROM memory_prompt_settings WHERE setting_id IN (${placeholders})`).all(...ids);
    }
    listMemoryPromptSettings(filter) {
        const conds = [];
        const args = [];
        if (filter.memoryPromptId) {
            conds.push("memory_prompt_id = ?");
            args.push(filter.memoryPromptId);
        }
        if (filter.targetType) {
            conds.push("target_type = ?");
            args.push(filter.targetType);
        }
        if (filter.teamId) {
            conds.push("team_id = ?");
    ...
    upsertMemoryPromptSettings(records, logs) {
        this.db.exec("BEGIN IMMEDIATE");
        try {
      ...
      `);
            for (const record of records) {
                upsert.run(record.setting_id, record.target_type, record.team_id ?? null, record.agent_id ?? null, record.layer, record.memory_prompt_id, record.updated_by ?? null, record.updated_at_ms);
            }
    ...
    clearMemoryPromptSettings(ids, logs) {
        if (ids.length === 0)
            return;
        this.db.exec("BEGIN IMMEDIATE");
        try {
    ...
    insertMemoryPromptSettingLogs(logs) {
    ...
    `);
        for (const log of logs) {
            insert.run(log.setting_log_id, log.target_type, log.team_id ?? null, log.agent_id ?? null, log.layer, log.action, log.reason, log.before_memory_prompt_id ?? null, log.after_memory_prompt_id ?? null, log.operator_id ?? null, log.operated_at_ms);
        }
    }
    deleteMemoryPrompts(ids, operatorId) {
        ...
        const cleared = { instance: 0, team: 0, agent: 0 };
        const placeholders = ids.map(() => "?").join(",");
        const settings = this.db.prepare(`SELECT * FROM memory_prompt_settings WHERE memory_prompt_id IN (${placeholders})`).all(...ids);
        const now = Date.now();
        const logs = settings.map((setting) => {
    ...
    queryMemoryPromptSettingLogs(filter) {
        const conds = [];
        const args = [];
        if (filter.memoryPromptId) {
        ...
        if (filter.startTimeMs !== undefined) {
            conds.push("operated_at_ms >= ?");
            args.push(filter.startTimeMs);
        }
        if (filter.endTimeMs !== undefined) {
            conds.push("operated_at_ms <= ?");
            args.push(filter.endTimeMs);
        }
        const where = conds.length > 0 ? `WHERE ${conds.join(" AND ")}` : "";
        const order = filter.timeOrder === "asc" ? "ASC" : "DESC";
        const limit = Math.min(Math.max(filter.limit ?? 20, 1), 100);
