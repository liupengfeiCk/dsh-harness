...
export class LocalStateBackend {
     ...
     */
    k(instanceId, sessionId, teamId, agentId) {
        return `${instanceId}:${teamId || "_"}:${agentId || "_"}:${sessionId}`;
    }
    // ═══ Buffer ═══
    async appendBuffer(instanceId, sessionId, message, teamId, agentId) {
    ...
    async drainBuffer(instanceId, sessionId, teamId, agentId) {
        const key = this.k(instanceId, sessionId, teamId, agentId);
    ...
    async getBufferLength(instanceId, sessionId, teamId, agentId) {
        return this.buffers.get(this.k(instanceId, sessionId, teamId, agentId))?.length ?? 0;
    }
    // ═══ Session State ═══
    async getSessionState(instanceId, sessionId, teamId, agentId) {
        return this.sessionStates.get(this.k(instanceId, sessionId, teamId, agentId)) ?? null;
    }
    async updateSessionState(instanceId, sessionId, patch, teamId, agentId) {
        const key = this.k(instanceId, sessionId, teamId, agentId);
    ...
    async deleteSessionState(instanceId, sessionId, teamId, agentId) {
        const key = this.k(instanceId, sessionId, teamId, agentId);
    ...
    async listActiveSessions(instanceId) {
        ...
        for (const key of this.sessionStates.keys()) {
            if (key.startsWith(prefix)) {
                ...
                if (parts.length >= 4)
                    sessions.push(parts.slice(3).join(":"));
            }
        }
        return sessions;
    }
    // ═══ Timer ═══
    async setTimer(instanceId, member, fireAtMs) {
    ...
    async setTimerIfEarlier(instanceId, member, fireAtMs) {
        const existing = this.timers.get(`${instanceId}:${member}`);
        if (existing && fireAtMs >= existing.fireAtMs)
            return false;
        await this.setTimer(instanceId, member, fireAtMs);
        return true;
    }
    async removeTimer(instanceId, member) {
        const key = `${instanceId}:${member}`;
        const existing = this.timers.get(key);
        if (existing?.handle)
            clearTimeout(existing.handle);
    ...
    async getExpiredTimers(instanceId, nowMs) {
        const prefix = `${instanceId}:`;
        const expired = [];
    ...
    async enqueueTask(task) {
        const idx = this.taskQueue.findIndex((t) => t.priority > task.priority || (t.priority === task.priority && t.createdAt > task.createdAt));
        if (idx === -1)
    ...
    async consumeTask(_workerId, blockMs) {
        ...
        return new Promise((resolve) => {
            const timer = setTimeout(() => {
                ...
                if (idx >= 0)
                    this.consumeWaiters.splice(idx, 1);
                resolve(null);
            }, blockMs);
            timer.unref();
    ...
    async destroy() {
        ...
        this.timers.clear();
        for (const w of this.consumeWaiters) {
