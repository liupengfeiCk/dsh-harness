...
function resolveProfileScope(options) {
...
function profileMatchesScope(record, scope, isolation) {
    if (scope === DEFAULT_PROFILE_SCOPE && !isolation)
        return true;
    if (record.id === buildProfileStableId(scope, record.type, record.filename))
        return true;
    if (!isolation)
        return false;
    if (isolation.teamId) {
...
async function refreshPersonaNavigation(dataDir, storage) {
    // Read persona body
    let body;
    if (storage) {
...
export async function listLocalProfiles(dataDir, storage, options) {
    const profiles = [];
...
export async function pullProfilesToLocal(dataDir, store, logger, storage, options) {
...
export async function syncLocalProfilesToStore(dataDir, store, baselineMap, logger, storage, options) {
...
export async function ensureL2L3Local(dataDir, store, logger, storage, options) {
    if (!store.pullProfiles)
        return new Map();
