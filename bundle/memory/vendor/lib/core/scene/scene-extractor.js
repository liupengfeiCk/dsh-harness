...
export class SceneExtractor {
    ...
    constructor(opts) {
        ...
        // NOTE: 原 fallback 为 OpenClaw 专用 CleanContextRunner（已随设计文档 §8.2
        // 裁切）。DSH 适配层始终注入 host-neutral LLMRunner，缺 runner 时报错。
        if (!opts.llmRunner) {
            throw new Error(`${TAG} L2 scene extraction requires an LLMRunner — DSH host must inject one ` +
    ...
    async extract(memories) {
        ...
        try {
            ...
            if (this.storage) {
                ...
                const fs = await import("node:fs/promises");
                allFiles = (await fs.default.readdir(sceneBlocksDir)).filter((f) => f.endsWith(".md"));
            }
        ...
        if (this.instanceId && this.logger) {
            ...
            // NOTE: 原 L2 延迟 / 场景变化评测指标（reportL2LatencyMetrics，Kafka
            // 计费观测）已随多租户/计费裁切移除（设计文档 §8.4）。
        }
