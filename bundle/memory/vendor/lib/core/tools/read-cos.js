 ...
 */
export async function executeReadCos(params, storage, logger) {
    const { path } = params;
    ...
    try {
        ...
        const content = obj.content.toString(encoding);
...
export const READ_COS_TOOL_SCHEMA = {
    ...
    properties: {
        path: {
            type: "string",
            description: "Full relative key of the file to read. " +
...
export const READ_COS_TOOL_DESCRIPTION = "Read a file from the memory storage system by its full relative key. " +
