...
export async function executeConversationSearch(params) {
    const { query, limit, sessionKey: sessionFilter, filter: isolationFilter, vectorStore, embeddingService, logger, } = params;
    ...
    if (vectorStore.getCapabilities().nativeHybridSearch && vectorStore.searchL0Hybrid) {
        ...
        const results = await vectorStore.searchL0Hybrid(isolationFilter ? { query, topK: candidateK, filter: isolationFilter } : { query, topK: candidateK });
        ...
        logger?.debug?.(`${TAG} RESULT (strategy=native-hybrid): returning ${trimmed.length} messages ` +
