...
export async function executeMemorySearch(params) {
    const { query, limit, type: typeFilter, scene: sceneFilter, filter: isolationFilter, vectorStore, embeddingService, logger, } = params;
    ...
    if (vectorStore.getCapabilities().nativeHybridSearch && vectorStore.searchL1Hybrid) {
        ...
        const results = await vectorStore.searchL1Hybrid(isolationFilter ? { query, topK: candidateK, filter: isolationFilter } : { query, topK: candidateK });
        ...
        logger?.debug?.(`${TAG} RESULT (strategy=native-hybrid): returning ${trimmed.length} memories ` +
    ...
    const [ftsItems, vecItems] = await Promise.all([
        ...
        (async () => {
            ...
            try {
                ...
                const vecResults = isolationFilter
