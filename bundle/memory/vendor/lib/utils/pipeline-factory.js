...
import { buildGenerationLogIdentity, buildGenerationProvenance, buildPromptGenerationRef, MemoryGenerationLogStore, } from "../core/memory-generation-log/store.js";
import { buildMemoryGenerationRefId, } from "../core/memory-generation-log/types.js";
...
import { DEFAULT_PROFILE_SCOPE, buildProfileIsolationScope, parseProfileIsolationScope, pullProfilesToLocal, listLocalProfiles, syncLocalProfilesToStore, } from "../core/profile/profile-sync.js";
...
function buildIsolationScope(ctx) {
    return buildProfileIsolationScope(ctx);
}
export function buildProfileL2Key(ctx) {
    ...
    if (storage) {
        try {
            ...
            for (const entry of result.entries) {
                ...
                if (encoded)
                    scopes.add(decodeURIComponent(encoded));
            }
        }
        catch (err) {
 ...
 */
export function initDataDirectories(dataDir, storage) {
    if (storage)
        return; // COS mode: no local directories needed
    ...
    return async ({ sessionKey }) => {
        ...
        try {
            ...
            if (vectorStore && !vectorStore.isDegraded()) {
                ...
                const jsonlGroups = await readConversationMessagesGroupedBySessionId(sessionKey, pluginDataDir, runnerState.last_l1_cursor || undefined, logger, L1_BATCH_QUERY);
            ...
            logger.info(`${TAG} [l1] Processing ${totalMessages} L0 messages across ${groups.length} sessionId group(s) ` +
                `for session ${sessionKey} (queried=${queriedCount}, sliceEnd=${sliceEnd}, ` +
                `hasMore=${hasMore}, hasFullBacklog=${hasFullBacklog})`);
            let totalExtracted = 0;
            let totalStored = 0;
            let lastSceneName;
            const profileScopes = new Set();
 ...
 */
export function createPersister(pluginDataDir, logger, storage) {
    return async (states) => {
...
export function createL2Runner(opts) {
    ...
    return async (sessionKey, cursor) => {
        ...
        logger.debug?.(`${TAG} [L2] session=${sessionKey}, profile=${profileFilter ? buildIsolationScope(profileFilter) : "(legacy-session)"}, updatedAfter=${cursor ?? "(full)"}`);
        ...
        for (const groupRecords of grouped.values()) {
            ...
            if (repaired) {
                logger.warn(`${TAG} [L2] ⚠️ Checkpoint counter regression detected and repaired ` +
            ...
            if (vectorStore && supportsProfileSyncWrite(vectorStore)) {
                changedProfiles = (await syncLocalProfilesToStore(groupDataDir, vectorStore, groupBaseline, logger, groupStorage, groupProfileOptions)) ?? changedProfiles;
            }
            ...
            await writeGenerationProvenanceBestEffort({
                layer: "l2",
                logger,
                writeLog: async () => l2LogStore.write(l2Log, l2Identity.key),
                writeRefs: vectorStore?.upsertMemoryGenerationRefs && changedProfiles.length > 0
                    ? async () => vectorStore.upsertMemoryGenerationRefs(changedProfiles.map((profile) => ({
...
export function createL3Runner(opts) {
    ...
    return async () => {
        ...
        const l3PromptTargets = executionScopes.map((scope) => {
            ...
            return { teamId: isolation.teamId, agentId: isolation.agentId, layer: "l3" };
        });
        ...
        for (const scope of executionScopes) {
            ...
            if (vectorStore && supportsProfileSyncWrite(vectorStore)) {
                changedProfiles = (await syncLocalProfilesToStore(scopedDir, vectorStore, profileBaseline, logger, scopedStore, profileOptions)) ?? changedProfiles;
            }
            if (changedProfiles.length === 0) {
            ...
            await writeGenerationProvenanceBestEffort({
                layer: "l3",
                logger,
                writeLog: async () => l3LogStore.write(l3Log, l3Identity.key),
                writeRefs: vectorStore?.upsertMemoryGenerationRefs && changedProfiles.length > 0
                    ? async () => vectorStore.upsertMemoryGenerationRefs(changedProfiles.map((profile) => ({
...
// NOTE: 原 V2 StateBackend-based pipeline factory（createStatefulPipelineManager，
// 基于 Redis 分布式状态后端）已随 state/ 分布式后端裁切移除（设计文档 §8.4）。
// DSH 单机部署仅使用本地 LocalStateBackend + MemoryPipelineManager。
