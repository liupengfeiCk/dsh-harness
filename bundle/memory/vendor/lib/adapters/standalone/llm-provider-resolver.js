 ...
 */
export function resolveStandaloneLlmForRuntime(llm, instanceId) {
    const provider = llm.provider ?? "openai";
    if (provider !== "proxy")
        return llm;
    if (!llm.baseUrl) {
        throw new LlmProviderResolveError("llm.provider=proxy 需要 llm.baseUrl 指向 context_proxy 根 URL");
    }
    if (!instanceId || !instanceId.trim()) {
        throw new LlmProviderResolveError("llm.provider=proxy 需要非空 instanceId，无法拼出 /proxy/<iid>/v1 路径");
    }
    ...
    if (useSystemUserKey) {
        ...
        if (!envKey) {
            throw new LlmProviderResolveError("llm.provider=proxy 需要 memory 系统用户 key —— " +
                "请在 yaml metadata.systemUser.memory 或 env TDAI_MEMORY_SYSTEM_USER_KEY 配置");
        }
        if (!MEMORY_USER_KEY_RE.test(envKey)) {
            throw new LlmProviderResolveError("memory 系统用户 key 必须匹配 sk-mem-[A-Za-z0-9_-]{32}");
        }
        effectiveApiKey = envKey;
    }
    if (!effectiveApiKey) {
        throw new LlmProviderResolveError("llm.provider=proxy 且 useMemorySystemUserKey=false 时必须显式 llm.apiKey");
    }
