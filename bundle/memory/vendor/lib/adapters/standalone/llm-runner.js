 ...
 */
function buildTelemetryMetadata(params) {
    const meta = {
 ...
 */
function createReadOnlyTools(_workspaceDir, _logger) {
    return {};
}
// ============================
// StandaloneLLMRunner
...
export class StandaloneLLMRunner {
    ...
    async run(params) {
        ...
        // AI SDK v6 默认走 /chat/completions（兼容 DeepSeek/Qwen 等 OpenAI-compatible
        // 后端）；v6 已移除 v5 的 `compatibility: "compatible"` 选项（该选项在 v6
        // 中不再存在，直接省略即可）。
        ...
        try {
            ...
            const result = await generateText({
                ...
                ...(tools && Object.keys(tools).length > 0
                    ? { tools: tools, stopWhen: stepCountIs(maxIterations) }
                    : {}),
                ...
                experimental_telemetry: {
                    ...
                    // AI SDK v6 metadata 需要 otel AttributeValue 形状；buildTelemetryMetadata
                    // 返回 Record<string, unknown>，此处收窄为 AttributeValue 联合类型。
                    metadata: buildTelemetryMetadata(params),
                },
            });
            const text = (result.text ?? "").trim();
            const totalMs = Date.now() - runStartMs;
            // 暴露 token usage 到 side-channel（供上层读取）
            // NOTE: AI SDK v6 的 LanguageModelUsage 字段由 v5 的 promptTokens/
            // completionTokens 更名为 inputTokens/outputTokens，此处已适配。
            if (result.usage) {
            ...
            for (const step of result.steps) {
                ...
                if (calls.length > 0) {
                    const callSummary = calls.map((tc) => `${tc.toolName}(${JSON.stringify(tc.input).slice(0, 120)})`).join(", ");
                    this.logger?.debug?.(`${TAG} step[${step.stepNumber}] toolCalls: ${callSummary}`);
                }
                if (textLen > 0) {
                    this.logger?.debug?.(`${TAG} step[${step.stepNumber}] text: ${textLen} chars, finishReason=${step.finishReason}`);
                }
                if (calls.length === 0 && textLen === 0) {
                    this.logger?.debug?.(`${TAG} step[${step.stepNumber}] empty (no tools, no text), finishReason=${step.finishReason}`);
                }
            }
            // Metric
