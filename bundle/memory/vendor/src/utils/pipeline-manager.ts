...
export class MemoryPipelineManager {
  ...
  private async runL1(sessionKey: string): Promise<void> {
    ...
    state.conversation_count = 0;
    this.advanceWarmupThreshold(state);
    const l2Keys = (runnerResult as { profileScopes?: string[] } | undefined)?.profileScopes?.length
      ? (runnerResult as { profileScopes: string[] }).profileScopes
      : [sessionKey];
  ...
  private async runL2(sessionKey: string): Promise<void> {
    ...
    const wasSkipped = (result as { skipped?: boolean } | undefined)?.skipped === true;
    ...
    const resultCursor = (result as { latestCursor?: string } | undefined)?.latestCursor;
    if (resultCursor) {
