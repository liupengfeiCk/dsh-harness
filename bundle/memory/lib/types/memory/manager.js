...
import { RoleBindingStore } from "./binding-store.js";
import { readMemorySettings } from "../config/index.js";
/** The memory management facade. */
export class MemoryManager {
    memory;
    bindings;
    constructor(memory, dataDir) {
        this.memory = memory;
    ...
    async l1Entries(store, isolation) {
...
function scopeKeyOf(scope, isolation) {
    switch (scope) {
        case 'project':
...
function matchesProfileScope(profile, scopeKey) {
    return profile.filename.startsWith(`${scopeKey}/`);
}
/** A profile record as an asset entry. */
function profileEntry(layer, profile) {
    return {
