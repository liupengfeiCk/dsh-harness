 ...
 */
import { CompactionConfigError, NoCompressibleHistoryError } from "./types.js";
/** Whether `messages[index]` is a safe cut: it does not split a tool pair. */
export function isBalancedBoundary(messages, index) {
    if (index < 0 || index >= messages.length)
        return false;
    const message = messages[index];
    // A tool-result message must stay glued to its assistant tool call.
    if (message.role === 'tool' && message.toolResultForCallId !== undefined)
        return false;
 ...
 */
export function selectCompressibleRange(messages, retainTokens) {
 ...
 */
export function partitionLayers(messages, span, levels) {
    const [startIndex, endIndex] = span;
    const count = endIndex - startIndex + 1;
    if (count <= 0)
        throw new NoCompressibleHistoryError('cannot partition an empty span');
 ...
 */
export function resolveBudgets(windowTokens, compressionLineRatio, retentionRatio) {
    ...
    if (retentionRatio > compressionLineRatio) {
        throw new CompactionConfigError(`retentionRatio (${retentionRatio}) must not exceed compressionLineRatio (${compressionLineRatio})`);
    }
    return {
