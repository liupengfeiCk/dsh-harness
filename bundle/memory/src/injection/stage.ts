...
export const STAGE_ORDER = ['first-turn', 'pre-compression', 'compression', 'post-compression'] as const
export type InjectionStage = typeof STAGE_ORDER[number]

/** Transition guard helper: the legal stage that follows a compression. */
export function afterCompressionStage(stage: InjectionStage): InjectionStage {
  return stage === 'post-compression' ? 'post-compression' : 'compression'
}

/** The stage a fresh session begins in. */
export const INITIAL_STAGE: InjectionStage = 'first-turn'
...
export interface InjectionPlan {
  ...
  readonly snapshot?: { readonly text: string }
...
export class StageLedger {
  private readonly states = new Map<string, InjectionStage>()
