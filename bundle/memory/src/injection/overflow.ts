 ...
 */
export function createOverflowRescueHandler(deps: OverflowRescueDeps): OverflowRescueHandler {
  ...
  const retries = new WeakMap<object, number>()
  // session → agent, so a successful assistant message can reset the owner.
  const owners = new WeakMap<Session, object>()
  ...
  return {
    ...
    clearBySession(session) {
      ...
      if (agent !== undefined) retries.delete(agent)
    },
    async handler(payload, next) {
      const { agent, failure, signal } = payload
      if (failure.code !== CONTEXT_WINDOW_EXCEEDED_CODE || signal.aborted) return next()
      const owner = agent as unknown as object
      ...
      let layers: readonly unknown[] | null
      try {
