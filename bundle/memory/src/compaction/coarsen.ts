...
export function summaryBudget(layers: readonly LayerSummary[]): number {
 ...
 */
export function pickCoarsenPair(
  layers: readonly LayerSummary[],
  excludedCoarse?: ReadonlySet<number>,
): readonly [fineIndex: number, coarseIndex: number] | null {
 ...
 */
export function withinCap(summaries: readonly LayerSummary[], capTokens: number): boolean {
 ...
 */
export async function recursiveCoarsen(
  layers: readonly LayerSummary[],
  capTokens: number,
  ...
  let refusedCoarse = new Set<number>()
  ...
  for (;;) {
    ...
    const mergedRange: [number, number] = [
    ...
    const mergedLayer: LayerSummary = {
      level: mergedLevel,
      range: mergedRange,
      text: produced.text,
...
function materializeSpan(
  fine: LayerSummary,
  coarse: LayerSummary,
): { range: readonly [number, number]; messages: readonly { seq: number; tokens: number; text: string; role: 'user' }[] } {
  ...
  return {
    range,
    messages: [
      { seq: fine.range[0], tokens: fine.tokens, text: fine.text, role: 'user' as const },
      { seq: coarse.range[0], tokens: coarse.tokens, text: coarse.text, role: 'user' as const },
    ],
  }
}
