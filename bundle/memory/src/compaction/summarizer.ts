...
import { CompactionConfigError } from './types.ts'
...
export class LlmSummarizer implements Summarizer {
  constructor(
    private readonly llm: { stream: (options: unknown) => AsyncIterable<{ type?: string; text?: string }> },
    private readonly resolveOptions: (ctx?: { readonly system?: string; readonly tools?: unknown }) => {
  ...
  async summarize(
    ...
    const request = {
      ...
      messages: [
        ...replay,
        { role: 'user' as const, content: CONDENSE_INSTRUCTION },
      ],
    }
    let text = ''
 ...
 */
export function createPrefixAlignedSummarizer(
  llm: LlmStream,
  resolveRoute: (ctx?: { readonly system?: string; readonly tools?: unknown }) => {
    readonly provider: string
    readonly model: string
    readonly maxTokens: number
    readonly system?: string
