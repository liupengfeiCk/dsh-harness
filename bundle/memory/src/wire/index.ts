...
import type { Memory } from '../memory/service.ts'
import type { ScopeIsolation } from '../memory/manager.ts'
 ...
 */
export function registerMemoryWire(ctx: Context): void {
  ctx.inject(['connection', 'memory'], (connectionCtx) => {
    const connection = connectionCtx.connection
    const memory = connectionCtx.get('memory') as Memory
    const manager = new MemoryManager(memory, memory.hostAdapter.dataDirPath)
    connectionCtx.effect(
      () => connection.rpc.handle(
        MEMORY_CHANNEL,
        (endpoint, payload, signal) => dispatchMemory(manager, endpoint, payload, signal),
        { authority: AUTHORITY },
      ),
      'dsh-memory: /memory rpc channel',
    )
  })
}
...
async function assets(
  manager: MemoryManager | undefined,
  request: EndpointPayload<'assets'>,
): Promise<RpcResult<unknown>> {
  const isolation = cleanIsolation(request.isolation)
  if (manager === undefined) return ok({ scope: request.scope, isolation, l1: [], l2: [], l3: [] })
...
async function assetRead(
  manager: MemoryManager | undefined,
  request: EndpointPayload<'assetRead'>,
): Promise<RpcResult<unknown>> {
  if (manager === undefined) return fail(notFound(request.ref))
  const asset = await manager.readAsset(request.ref)
  if (asset === undefined) return fail(notFound(request.ref))
  return ok(asset)
}

async function assetDelete(
  manager: MemoryManager | undefined,
  request: EndpointPayload<'assetDelete'>,
): Promise<RpcResult<unknown>> {
  if (manager === undefined) return fail(notFound(request.ref))
...
async function bindRole(
  manager: MemoryManager | undefined,
  request: EndpointPayload<'bindRole'>,
): Promise<RpcResult<unknown>> {
  if (manager === undefined) return fail(noManager(request.roleId))
...
async function unbindRole(
  manager: MemoryManager | undefined,
  request: EndpointPayload<'unbindRole'>,
): Promise<RpcResult<unknown>> {
  if (manager === undefined) return fail(noManager(request.roleId))
...
async function roleBindings(
  manager: MemoryManager | undefined,
  request: EndpointPayload<'roleBindings'>,
): Promise<RpcResult<unknown>> {
  if (manager === undefined) return ok({ assets: [] })
  return ok({ assets: [...await manager.roleBindings(request.roleId)] })
}

async function status(
  manager: MemoryManager | undefined,
  request: EndpointPayload<'status'>,
): Promise<RpcResult<unknown>> {
  const isolation = cleanIsolation(request.isolation)
...
async function config(manager: MemoryManager | undefined): Promise<RpcResult<unknown>> {
  if (manager === undefined) {
    return ok({
      enabled: true,
      refinementPlan: '',
...
function cleanIsolation(isolation: EndpointPayload<'assets'>['isolation']): ScopeIsolation {
...
function notFound(ref: string): WireError {
  return {
...
function noManager(id: string): WireError {
  return {
