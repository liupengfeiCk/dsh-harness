...
import { FileSummaryStorage, NoCoverageError } from '../compaction/index.ts'
import type { LayerSummary } from '../compaction/types.ts'
import { sessionRawStore } from '../injection/raw-store.ts'
...
export interface RefinedDetail {
  ...
  readonly rawMessages: readonly { seq: number; text: string; role: string }[]
}
 ...
 */
export async function loadFineLayers(
  storage: { load(sessionId: string): Promise<LayerSummary[]> },
  sessionId: string,
  seq: number,
 ...
 */
function defaultRefine(storageBase?: string): RefineFn {
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const memory = (getTraceable(ctx, ctx) as Context).get('memory') as ConversationSearchLike | undefined
      if (memory === undefined) {
