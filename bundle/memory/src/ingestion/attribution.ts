...
/**
 * The `subagent-team/mode` session event shape (folded structurally). Kept as a
 * standalone structural type rather than an intersection with `SessionEvent`:
 * no `SessionEvent` union member has `type === 'subagent-team/mode'`, so an
 * `X & { type: 'subagent-team/mode'; ... }` intersection would collapse to
 * `never`. The event is decoded from the log via an `as unknown` cast instead.
 */
type TeamModeEvent = {
  type: 'subagent-team/mode'
  data: { mode: 'standard' | 'team'; team?: string }
}
 ...
 */
export function foldTeamMode(events: readonly SessionEvent[]): string {
  for (let index = events.length - 1; index >= 0; index -= 1) {
    ...
    const candidate = event as unknown as TeamModeEvent
...
export interface TasksLike {
  list(filter?: { includeCompleted?: boolean }): readonly {
    id: string
    sessions: readonly { sessionId: string; roleId?: string }[]
  }[]
}
 ...
 */
function taskRoleForSession(
  ctx: Context,
  sessionId: string,
): { task?: string; role?: string } {
  const tasks = (ctx as unknown as { get(name: string): TasksLike | undefined }).get('tasks')
 ...
 */
export function deriveTurnAttribution(ctx: Context, session: Session): TurnAttribution {
 ...
 */
export function encodeSessionKey(attribution: TurnAttribution): string {
  const parts = [
    attribution.team,
    attribution.role,
    attribution.project,
