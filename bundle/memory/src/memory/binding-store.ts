 ...
 */
export async function loadBindings(filePath: string): Promise<BindingsDocument> {
  try {
    const text = await readFile(filePath, 'utf8')
    const parsed = JSON.parse(text) as Partial<BindingsDocument>
 ...
 */
export async function saveBindings(filePath: string, document: BindingsDocument): Promise<void> {
...
export class RoleBindingStore {
  private document: BindingsDocument = { roles: {} }
  ...
  async assetsFor(roleId: string): Promise<readonly string[]> {
    await this.load()
    return this.document.roles[roleId] ?? []
  }

  /** Bind one asset to a role (idempotent; keeps the existing order). */
  async bind(roleId: string, assetRef: string): Promise<void> {
    await this.load()
    const current = this.document.roles[roleId] ?? []
    if (current.includes(assetRef)) return
    this.document = {
  ...
  async unbind(roleId: string, assetRef: string): Promise<void> {
    await this.load()
    const current = this.document.roles[roleId]
