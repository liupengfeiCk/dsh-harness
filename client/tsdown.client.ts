...
/**
 * Repo root anchor for CSS virtual ids: the emitted region markers must carry
 * repository-relative paths only — an absolute path would bake the building
 * machine's local directory layout into the published artifact.
 */
...
export function clientBundle(
  ...
  return ({ env }) => {
    ...
    const client = clientConfig(id, clientSourceEntry(face, options.clientDir))
...
/**
 * Resolve the browser-bundle entry for one UI half: the tsx source in the
 * client (or a bundle's `ui`) subdirectory during a dev/ambiguous build, and
 * the tsc-emitted node half after a typecheck pass.
 * @param face - the current build face.
 * @param clientDir - the client source subdirectory (defaults to `client`).
 * @returns the entry module specifier for the browser half.
 */
export function clientSourceEntry(face: BuildFace | undefined, clientDir: string): string {
  const dir = clientDir || 'client'
  return face === undefined ? `src/${dir}/index.ts` : `lib/types/${dir}/index.js`
}
...
interface ClientBundleOptions {
  ...
  /** The client source subdirectory (defaults to `client`); a bundle's UI half lives in `ui`. */
...
export function buildFace(value: unknown): BuildFace {
  ...
  return {
    ...
    plugins: [{
      ...
      resolveId(source: string, importer: string | undefined) {
        ...
        return CSS_VIRTUAL_PREFIX + relative(REPO_ROOT, abs) + CSS_VIRTUAL_SUFFIX
      },
      async load(virtualId: string) {
        if (!virtualId.startsWith(CSS_VIRTUAL_PREFIX)) return null
        // The virtual id carries a repo-relative path (see resolveId); resolve
        // it back to the physical stylesheet for reads and watch registration.
        const fileId = resolvePath(REPO_ROOT, virtualId.slice(CSS_VIRTUAL_PREFIX.length, -CSS_VIRTUAL_SUFFIX.length))
