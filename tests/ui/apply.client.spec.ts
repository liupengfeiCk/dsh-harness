...
import { AGENT_PRESET_EDIT_CHANNEL } from '../../src/ui/wire-client.ts'
import { AgentPresetLabel } from '../../src/ui/AgentPresetLabel.tsx'
import type { AgentPresetLabelInjected } from '../../src/ui/AgentPresetLabel.tsx'
import { AgentPresetRow } from '../../src/ui/AgentPresetRow.tsx'
import type { AgentPresetRowInjected } from '../../src/ui/AgentPresetRow.tsx'
import { AgentPresetSection } from '../../src/ui/AgentPresetSection.tsx'
import type { AgentPresetSectionInjected } from '../../src/ui/AgentPresetSection.tsx'
import { AgentPresetSeat } from '../../src/ui/AgentPresetSeat.tsx'
import type { AgentPresetSeatInjected } from '../../src/ui/AgentPresetSeat.tsx'
  ...
  ctx.provide('connection', {
    ...
    rpc: {
      call: (channel: string, endpoint: string, payload: unknown) => {
        if (channel !== AGENT_PRESET_EDIT_CHANNEL) {
          return Promise.reject(new Error(`unexpected rpc channel ${JSON.stringify(channel)}`))
        }
        calls.push(`edit:${endpoint}`)
        const preset = (payload as { agentPreset?: string }).agentPreset ?? ''
        if (endpoint === 'readEditable') {
          return Promise.resolve({ ok: true as const, value: { agentPreset: preset, delegation: [], tools: [], catalog: [] } })
        }
        if (endpoint === 'update') {
          return Promise.resolve({ ok: true as const, value: { agentPreset: preset } })
        }
