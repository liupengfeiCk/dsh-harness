...
function createMergeHandler(deps) {
	return async (payload, next) => {
		...
		if (selection.planId === void 0) return next();
		...
		if (warnedExtra.length > 0) deps.logger.warn(`model-plans: plan "${selection.planId}" carries non-native params (${warnedExtra.join(", ")}) that land on LlmCallConfig.extra — the active provider adapter may not forward them (pi-ai does not)`);
		return config;
	};
}
//#endregion
export { KNOWN_KEYS, createMergeHandler, mergePlanConfig };
