window.__ModuleLoader__.load({
	...
	factory: (require) => {
							...
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [(0, react_jsx_runtime.jsx)("span", {
									...
									children: [(0, react_jsx_runtime.jsx)("option", {
										value: "",
										disabled: true,
										children: bodies.length === 0 ? t("noBodies") : t("roleBodyPlaceholder")
									}), bodies.map((body) => (0, react_jsx_runtime.jsx)("option", {
										value: body,
										children: body
									}, body))]
								})]
			...
			return (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Modal, {
				...
				children: (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						(0, react_jsx_runtime.jsx)(RolesEditor, {
							roles: draft.roles,
							bodies: state.bodies,
							t,
							setRoleField,
		...
		function RolesEditor(props) {
			...
			return (0, react_jsx_runtime.jsxs)("section", {
				...
				children: [
					...
					roles.map((role, index) => (0, react_jsx_runtime.jsx)(RoleEditorRow, {
						role,
						index,
						bodies,
						t,
		...
		function TeamSection(props) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					state.detail !== null ? (0, react_jsx_runtime.jsx)(DetailDialog, {
						detail: state.detail,
						bodies: state.bodies,
						t,
		...
		*/
		...
		const en = {
			nav: "Teams",
			sectionIntro: "Teams are your own role rosters (\"编制表\"), stored independently from subagents. Each role binds a soul (the role's own prompt) to a body (a user-defined subagent id that bounds the role's capability surface). A team-shaped deployment delegates to roles by name.",
			...
			detailIntro: "Each role binds a soul (its own prompt, injected as the child's persona) to a body (a subagent id bounding what the role may do). Memory decides whether the role is a fresh child per call or a durable one that remembers earlier work.",
			save: "Save",
			saving: "Saving…",
			cancel: "Cancel",
			close: "Close",
		...
		const zh = {
			nav: "团队",
			sectionIntro: "团队是你自定义的角色编制表，与子代理分开存储。每个角色把一个灵魂（角色自己的提示词）绑定到一个身体（一个用户自定义子代理 id，界定该角色的能力边界）。团队形态的部署按名称把任务委派给角色。",
			loading: "正在加载团队…",
			error: "无法加载团队。",
			...
			detailTitle: "编辑团队",
			detailIntro: "每个角色把灵魂（自己的提示词，注入为子代理的 persona）绑定到一个身体（界定该角色能力范围的子代理 id）。记忆模式决定该角色是每次调用都全新、还是保留可续对话的长期角色。",
			save: "保存",
			saving: "正在保存…",
			cancel: "取消",
			close: "关闭",
		...
		*/
		function apply$1(ctx) {
			...
			const sectionInjected = () => ({
				...
				setDetailRoleField: (index, field, value) => {
					section.setDetailRoleField(index, field, value);
				},
				addDetailRole: () => {
					section.addDetailRole();
				},
				removeDetailRole: (index) => {
					section.removeDetailRole(index);
				},
