...
import { Button, IconBrowseOutline16, IconCopyOutline16, IconEditOutline16, IconFolderOpenOutline16, IconPlusOutline16, IconTrashOutline16, Modal, Tooltip, } from '@deepseek-ai/dsh-client-ui-primitives';
import { createBlocker, INHERIT_MODEL_VALUE, } from "./section-store.js";
...
export function SubagentPresetSection(props) {
    ...
    return (_jsxs("div", { className: css.section, children: [_jsx("h2", { className: css.title, children: t('nav') }), _jsx("p", { className: css.intro, children: t('sectionIntro') }), _jsx("ul", { className: css.cards, children: state.rows.map(row => (_jsx(SubagentRowView, { row: row, t: t, onToggle: props.toggle, onView: props.beginView, onCopy: props.beginCreate, onEdit: props.beginEdit, onDelete: props.confirmDelete, onOpen: props.openLocation, hasDocument: state.hasDocument, revealedPath: state.revealedPaths[row.id] }, row.id))) }), factory === undefined ? null : (_jsxs("button", { type: "button", className: css.creatorButton, disabled: !state.authorable, onClick: () => props.beginCreate(factory.id), children: [_jsx(IconPlusOutline16, {}), t('create')] })), _jsx(CreateDialog, { state: state, t: t, setCreateId: props.setCreateId, confirm: () => void props.confirmCreate(), cancel: props.cancelCreate }), state.view !== null
