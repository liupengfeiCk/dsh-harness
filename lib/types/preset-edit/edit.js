...
function rowBlock(lines, row) {
    ...
    while (end < lines.length) {
        ...
        if (indentOf(line) < row.keyIndent)
            break;
        end += 1;
    }
    return { start, end };
 ...
 */
function locateRow(lines, id) {
    for (let index = 0; index < lines.length; index += 1) {
        const line = lines[index];
        if (line === undefined)
            continue;
        const match = rowId(line);
        if (match === undefined || match.indent !== 0 || match.id !== id)
            continue;
...
function findConfigLine(lines, start, end) {
    for (let index = start; index < end; index += 1) {
        const line = lines[index];
        if (line === undefined)
            continue;
        if (bodyOf(line) === 'config:')
            return index;
    }
    return -1;
}
/** The present key lines of a `config:` block. */
function configKeys(lines, config) {
    ...
    while (end < lines.length) {
        ...
        if (line === undefined)
            break;
        const body = bodyOf(line);
        if (body === '' || indentOf(line) < keyIndent)
            break;
 ...
 */
function renderKeyLines(key, value, indent) {
 ...
 */
function editConfigKeys(lines, block, edits, presetId, allowed) {
 ...
 */
const DELEGATION_WRITABLE_KEYS = [
...
const UNKNOWN_PROVIDER_FIELDS = { hasDepthLimit: false, hasContinuable: false };
...
function isLiteralDisabled(value) {
...
function locateNestedRow(lines, group, id) {
        ...
        for (;;) {
            ...
            for (const entry of entries) {
                ...
                if (!entry.isDirectory() && !entry.isSymbolicLink())
                    continue;
                if (!entry.name.startsWith('dsh-tool-'))
                    continue;
 ...
 */
export function readEditableFields(content, available = []) {
    ...
    const instances = Array.isArray(nested) ? nested : [];
    ...
    for (const pkg of available) {
        if (pkg.description !== undefined)
            descriptions.set(pkg.name, pkg.description);
    }
    const tools = [];
    const addToolRow = (id, name, disabled) => {
        if (typeof id !== 'string' || typeof name !== 'string')
            return;
        if (!name.startsWith('@deepseek-ai/dsh-tool-'))
            return;
        const state = isJsExpr(disabled)
            ? 'expr'
    ...
    for (const row of rows) {
        ...
        if (Array.isArray(row.config)) {
            for (const nestedRow of row.config) {
    ...
    const catalog = available.map(pkg => ({
        name: pkg.name,
        toolNames: [],
 ...
 */
export function applyEdits(content, edits, presetId) {
    ...
    try {
        ...
        throw new InvalidPresetEditsError(presetId, `this preset is not valid YAML: ${full.replace(/\n[\s\S]*$/, '')}`);
    }
    ...
    if (tools !== undefined && Object.keys(tools).length > 0) {
        for (const [id, edit] of Object.entries(tools)) {
            ...
            if (loc.disabled === 'expr') {
                throw new InvalidPresetEditsError(presetId, `the tool row "${id}" is disabled by a loader expression, not a user toggle`);
            }
    ...
    try {
        ...
        const full = error instanceof Error ? error.message : String(error);
        throw new InvalidPresetEditsError(presetId, `the edit made the composition unparseable: ${full.replace(/\n[\s\S]*$/, '')}`);
    }
    return edited;
}
/** Locate a tool row (top-level or nested) and its disabled line, if any. */
function locateToolRow(lines, id) {
    // Top-level scan.
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (line === undefined)
            continue;
        const match = rowId(line);
        if (match === undefined || match.indent !== 0)
            continue;
...
function findDisabledLine(lines, row) {
    const { start, end } = rowBlock(lines, row);
    for (let index = start; index < end; index += 1) {
        const line = lines[index];
        if (line === undefined)
            continue;
        if (indentOf(line) === row.keyIndent && bodyOf(line).startsWith('disabled:'))
            return index;
    }
    return -1;
}
/** Read how a row's disabled line reads, at the given index (-1 = absent). */
function disabledStateAt(lines, disabledLine) {
    if (disabledLine === -1)
        return false;
...
function setToolDisabled(lines, row, disabled) {
    const out = [...lines];
...
function hasToolName(lines, name) {
    const quoted = `'${name}'`;
    for (let index = 0; index < lines.length; index += 1) {
        const line = lines[index];
        if (line === undefined)
            continue;
        if (line.trim().startsWith('name:') && line.includes(quoted))
            return true;
    }
    return false;
}
 ...
 */
function removeToolRow(lines, row) {
 ...
 */
function insertToolRows(lines, rows) {
