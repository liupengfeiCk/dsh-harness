...
export function registerAgentPresetEditWire(ctx) {
    ...
    const tryRegister = () => {
        ...
        if (connection === undefined)
            return;
        ctx.effect(() => connection.rpc.handle(AGENT_PRESET_EDIT_CHANNEL, (endpoint, payload, signal) => dispatchAgentPresetEdit(ctx.get('agentPresets'), endpoint, payload, signal), { authority: AUTHORITY }), 'dsh-preset-edit: /agent-preset-edit rpc channel');
    };
    tryRegister();
