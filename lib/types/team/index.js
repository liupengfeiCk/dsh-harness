...
export class Teams extends Service {
    ...
    async resolveRole(teamId, roleId) {
        ...
        if (subagents === undefined) {
            throw new Error(`team-presets: team "${teamId}" role "${roleId}" binds subagent "${role.subagent}" but no subagent `
        ...
        if (subagent.broken !== undefined) {
            throw new Error(`team-presets: team "${teamId}" role "${roleId}" cannot be delegated: its subagent "${role.subagent}" `
     ...
     */
    async create(id, metadata, roles) {
        if ((await this.list()).some(team => team.id === id))
     ...
     */
    async update(id, metadata, roles) {
