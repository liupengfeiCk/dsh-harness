...
function RoleListRow(props) {
    ...
    return (_jsxs("li", { className: css.roleListRow, children: [_jsxs("button", { type: "button", className: css.roleListMain, disabled: !canEdit, onClick: () => onEdit(index), children: [_jsx("span", { className: css.roleListId, children: role.id === '' ? t('roleIdEmpty') : role.id }), _jsx("span", { className: css.roleListSummary, children: role.description === '' ? t('noRoleSummary') : role.description }), _jsxs("span", { className: css.roleListMeta, children: [_jsxs("span", { className: css.roleListBody, children: [_jsx("span", { className: css.roleListLabel, children: t('roleBodyLabelShort') }), _jsx("span", { className: css.roleListBodyValue, children: role.body === '' ? '—' : role.body })] }), _jsxs("span", { className: css.roleListMemory, children: [_jsx("span", { className: css.roleListLabel, children: t('roleMemoryLabelShort') }), _jsx("span", { className: css.roleListMemoryValue, children: role.memory === 'persistent' ? t('memoryPersistent') : t('memoryOneShot') })] })] })] }), _jsx(Tooltip, { label: t('removeRole'), side: "top", children: _jsx("button", { type: "button", className: `${css.iconButton} ${css.iconDanger}`, disabled: !canEdit || saving, onClick: () => onRemove(index), "aria-label": t('removeRole'), children: _jsx(IconTrashOutline16, {}) }) })] }));
}
...
function CreateRolesEditor(props) {
    ...
    return (_jsxs("section", { className: css.editorBlock, children: [_jsx("h4", { className: css.blockTitle, children: t('rolesLabel') }), roles.map((role, index) => (_jsx(CreateRoleRow, { role: role, index: index, bodies: bodies, t: t, onField: setRoleField, onRemove: removeRole, removable: roles.length > 1 }, index))), _jsxs("button", { type: "button", className: css.addRole, onClick: addRole, children: [_jsx(IconPlusOutline16, {}), t('addRole')] })] }));
}
...
function RoleEditDialog(props) {
    ...
    return (_jsx(Modal, { open: true, onClose: cancel, title: t('roleEditTitle'), closeLabel: t('close'), description: t('roleEditIntro'), className: `${css.dialog} ${css.roleEditDialog}`, contentClassName: css.dialogScroll, footer: (_jsxs(_Fragment, { children: [_jsx(Button, { variant: "outline", disabled: saving, onClick: cancel, icon: _jsx(IconChevronLeftOutline14, {}), children: t('roleEditBack') }), _jsx(Button, { disabled: saving || blocker !== undefined || !edit.dirty, onClick: confirm, children: saving ? t('saving') : t('roleEditSave') })] })), children: _jsxs("div", { className: css.roleEditForm, children: [_jsxs("div", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('roleIdLabel') }), _jsx("input", { className: css.input, value: draft.id, placeholder: t('roleIdPlaceholder'), onChange: e => setField('id', e.target.value) })] }), _jsxs("div", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('roleDescriptionLabel') }), _jsx("input", { className: css.input, value: draft.description, placeholder: t('roleDescriptionPlaceholder'), onChange: e => setField('description', e.target.value) })] }), _jsxs("div", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('roleBodyLabel') }), _jsxs("select", { className: `${css.input} ${css.select}`, value: draft.body, onChange: e => setField('body', e.target.value), children: [_jsx("option", { value: "", disabled: true, children: bodies.length === 0 ? t('noBodies') : t('roleBodyPlaceholder') }), bodies.map(body => _jsx("option", { value: body, children: body }, body))] })] }), _jsxs("div", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('roleMemoryLabel') }), _jsxs("select", { className: `${css.input} ${css.select}`, value: draft.memory, onChange: e => setField('memory', e.target.value), children: [_jsx("option", { value: "one-shot", children: t('memoryOneShot') }), _jsx("option", { value: "persistent", children: t('memoryPersistent') })] })] }), _jsxs("div", { className: css.field, children: [_jsx("span", { className: css.fieldLabel, children: t('rolePromptLabel') }), _jsx("textarea", { className: `${css.input} ${css.textarea} ${css.roleEditPrompt}`, value: draft.prompt, placeholder: t('rolePromptPlaceholder'), onChange: e => setField('prompt', e.target.value) })] }), message !== null ? _jsx("p", { className: css.error, children: message }) : null] }) }));
}
/**
...
export function TeamSection(props) {
    ...
    return (_jsxs("div", { className: css.section, children: [_jsx("h2", { className: css.title, children: t('nav') }), _jsx("p", { className: css.intro, children: t('sectionIntro') }), _jsx("ul", { className: css.cards, children: state.rows.map(row => (_jsx(TeamRowView, { row: row, t: t, onToggle: props.toggle, onDetail: props.beginDetail, onDelete: props.confirmDelete, onOpen: props.openLocation, hasDocument: state.hasDocument, revealedPath: state.revealedPaths[row.id] }, row.id))) }), _jsxs("button", { type: "button", className: css.creatorButton, disabled: !state.authorable, onClick: () => props.beginCreate(), children: [_jsx(IconPlusOutline16, {}), t('create')] }), _jsx(CreateDialog, { state: state, t: t, setCreateId: props.setCreateId, setCreateName: props.setCreateName, setRoleField: props.setCreateRoleField, addRole: props.addCreateRole, removeRole: props.removeCreateRole, confirm: () => void props.confirmCreate(), cancel: props.cancelCreate }), state.detail !== null
                ? (inRoleEdit
                    ? null
                    : (_jsx(DetailDialog, { detail: state.detail, bodies: state.bodies, t: t, setDetailName: props.setDetailName, setDetailDescription: props.setDetailDescription, beginRoleEdit: props.beginRoleEdit, addRole: props.addRoleInDetail, removeRole: props.removeRole, confirm: () => void props.confirmDetail(), cancel: props.closeDetail })))
                : null, inRoleEdit && state.detail !== null
                ? (_jsx(RoleEditDialog, { edit: roleEdit, bodies: state.bodies, t: t, setField: props.setRoleEditField, saving: state.detail.saving, confirm: () => void props.saveRoleEdit(), cancel: props.cancelRoleEdit }))
