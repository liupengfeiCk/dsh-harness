...
import { dshHomePath, expandHomePath } from "../home-path.js";
import { METADATA_FILE, readSubagentMetadata, renderSubagentMetadata } from "./metadata.js";
...
export class InvalidSubagentIdError extends Error {
    ...
    /** The rejected id. */
    subagentId) {
        super(`subagent-presets: subagent id ${JSON.stringify(subagentId)} must match ${String(SUBAGENT_ID)} — `
...
export class SubagentExistsError extends Error {
    ...
    subagentId) {
        super(`subagent-presets: subagent "${subagentId}" already exists — `
 ...
 */
export async function readEnabledOverrides() {
    let raw;
    try {
 ...
 */
export async function writeEnabledOverride(id, enabled) {
    const overrides = { ...await readEnabledOverrides(), [id]: enabled };
    await writeFileAtomic(dshHomePath(ENABLED_OVERRIDES_FILE), `${JSON.stringify(overrides, null, 2)}\n`, { mode: 0o600, dirMode: 0o700 });
}
/**
 ...
 */
export async function readComposition(subagent) {
 ...
 */
export async function writeComposition(subagent, content) {
 ...
 */
export async function createSubagent(roots, source, id) {
    if (!SUBAGENT_ID.test(id))
        throw new InvalidSubagentIdError(id);
 ...
 * writable root.
 */
export async function deleteSubagent(roots, subagent) {
 ...
 * writable root.
 */
export async function updateSubagent(roots, subagent, metadata) {
    if (subagent.trust !== 'user') {
        throw new SubagentNotWritableError(subagent.id, 'it ships with the deployment');
    }
    const dir = join(writableRoot(roots), subagent.id);
 ...
 * writable root.
 */
export async function setEnabledField(roots, subagent, enabled) {
    if (subagent.trust !== 'user') {
        throw new SubagentNotWritableError(subagent.id, 'it ships with the deployment');
    }
    const dir = join(writableRoot(roots), subagent.id);
 ...
 */
async function writeSubagentMetadata(dir, metadata) {
