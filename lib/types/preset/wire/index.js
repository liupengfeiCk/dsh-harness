...
import { SubagentNotWritableError, UnknownSubagentError, } from "../index.js";
import { canOpenDirectory, openDirectory } from "./opener.js";
 ...
 */
export async function dispatchSubagentPreset(registry, endpoint, payload, signal) {
    try {
        switch (endpoint) {
            case 'list':
 ...
 */
export function registerSubagentPresetWire(ctx) {
    ctx.inject(['connection'], (connectionCtx) => {
        const connection = connectionCtx.connection;
        connectionCtx.effect(() => connection.rpc.handle(SUBAGENT_PRESET_CHANNEL, (endpoint, payload, signal) => dispatchSubagentPreset(connectionCtx.get('subagentPresets'), endpoint, payload, signal), { authority: AUTHORITY }), 'dsh-subagent-preset: /subagent-preset rpc channel');
    });
}
/** A deployment with no subagent registry answers an empty roster, not an error. */
async function list(registry) {
...
async function read(registry, request) {
    if (registry === undefined)
        return fail(notFound(request.subagent));
    try {
...
async function create(registry, request) {
    if (registry === undefined)
        return fail(notFound(request.subagent));
    try {
...
async function openDocument(registry, request, signal) {
    if (registry === undefined)
        return fail(notFound(request.subagent));
    try {
        const resolved = await registry.resolve(request.subagent);
...
async function openDirectoryResult(directory, signal) {
    try {
...
async function remove(registry, request) {
    if (registry === undefined)
        return fail(notFound(request.subagent));
    try {
...
async function readEditable(registry, request) {
    if (registry === undefined)
        return fail(notFound(request.subagent));
    try {
...
async function update(registry, request) {
    if (registry === undefined)
        return fail(notFound(request.subagent));
    try {
        ...
        const enabledOnly = metadata !== undefined
            ...
            && 'enabled' in metadata;
        if (enabledOnly) {
            await registry.setEnabled(subagent, metadata.enabled);
            return ok({ subagent });
        }
        const staged = {};
        if (persona !== undefined)
            staged.persona = persona;
        await registry.updateComposition(subagent, {
...
function subagentFailure(subagent, error) {
