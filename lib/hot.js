...
var HotManager = class {
	...
	*/
	async upgrade(ctx, loader, packageName) {
		...
		if (!preflight.ok) throw new RestartRequiredError(`upgrade would change the package structure of "${packageName}" (${preflight.reason}) — restart to activate`);
		await this.unmount(packageName, ctx);
...
*/
function collectPackageLoadCacheUrls(loader, profileDir, packageName) {
	const internal = loader.internal;
	if (internal === void 0) return null;
	const loadCache = internal.loadCache;
	if (loadCache === null || typeof loadCache !== "object" || typeof loadCache.keys !== "function") return null;
	let packageReal;
	try {
...
*/
function preflightUpgrade(profileDir, packageName, loader) {
	let pkgDir;
	try {
...
*/
function preflightUpgradeStructure(pkgDir, loadedFiles, diskPackageJsonText) {
	for (const file of loadedFiles) {
		if (existsSync(file)) continue;
		return {
			ok: false,
			reason: `already-loaded module ${relative(pkgDir, file)} no longer exists on disk (nested node_modules layout changed)`
		};
	}
	let exportsField;
	try {
	...
	if (exportsField === void 0) return { ok: true };
	...
	for (const file of loadedFiles) {
		...
		return {
			ok: false,
			reason: `already-loaded module ${relative(pkgDir, file)} is no longer exported by the new package.json exports map`
		};
	}
	return { ok: true };
}
...
function collectExactExportedFiles(exportsField, pkgDir) {
	...
	if (typeof exportsField === "string") pushTarget(exportsField);
	else if (exportsField !== null && typeof exportsField === "object" && !Array.isArray(exportsField)) {
		for (const value of Object.values(exportsField)) if (typeof value === "string") pushTarget(value);
		else if (value !== null && typeof value === "object" && !Array.isArray(value)) {
			for (const leaf of Object.values(value)) if (typeof leaf === "string") pushTarget(leaf);
		}
	}
	return {
		files,
...
//#endregion
export { ActivationTimeout, HOT_DIR, HOT_MOUNT_TIMEOUT_MS, HOT_ROW_PREFIX, HotManager, clearNodeResolutionCache, clearPackageLoadCache, collectPackageLoadCacheUrls, preflightUpgrade, preflightUpgradeStructure };
