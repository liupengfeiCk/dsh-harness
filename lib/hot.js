...
var HotManager = class {
	...
	*/
	async mount(ctx, packageName) {
		...
		try {
			const loaderLike = ctx.loader;
			const internalAny = loaderLike?.internal;
			const imported = internalAny?.import ? await internalAny.import(packageName, pathToFileURL(this.hotDir).href + "/") : null;
			writeFileSync("/tmp/harness-hot-mount-debug.json", JSON.stringify({
				packageName,
				importedKeys: imported ? Object.keys(imported) : null,
				importedApplyType: imported ? typeof imported.apply : null,
...
//#endregion
export { ActivationTimeout, HOT_DIR, HOT_MOUNT_TIMEOUT_MS, HOT_ROW_PREFIX, HotManager, clearPackageLoadCache };
